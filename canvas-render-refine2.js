    ...
    return new Promise((resolve, reject) => {
      ...
      const _onSeeked = async function onSeeked() {
        ...
        function drawCanvasFrame(t) {
          ...
          // Clip GRADE (constant look filter) — the base pass, applied to the whole composited
          // frame by redrawing it through ctx.filter (same trick as the desaturate below). Reads
          // getGradeFilter() so it's the exact string the live preview uses → export == preview.
          // Colour grade needs only _supportsCtxFilter; the url() sharpen is appended only where
          // the canvas accepts it AND the SVG is present (else the colour grade still lands).
          if (_supportsCtxFilter) {
            const gBase = api.getGradeFilter?.(false);
            if (gBase) {
              const useSharpen = _supportsCtxFilterUrl && document.getElementById('clipSharpen');
              ctx.save();
              ctx.filter = useSharpen ? api.getGradeFilter(true) : gBase;
