  ...
  function initCanvas(item, opts = {}) {
    ...
    if (gpHold && gpVideo) {
      ...
      const _reframe = (window.wcReframe ||= {
        ...
        rebake()      { this.track = _smoothTrack(this.raw, this.cfg); return `[reframe] rebaked ${this.track.length} pts (smoothMs ${this.cfg.smoothMs}, deadzone ${this.cfg.deadzone})`; },
      ...
      function _applyReframe(srcSec) {
        if (!_reframe.enabled || _reframe._suspend || !(srcSec >= 0)) return;
