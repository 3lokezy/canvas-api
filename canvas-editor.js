  ...
  (function () {
    ...
    function scaleFrame(frame) {
      ...
      if (!wrap) return;
      const scale = Math.min(wrap.clientWidth / frame.offsetWidth, (wrap.clientHeight || Infinity) / frame.offsetHeight, 1);
    ...
    function makeOverlayInteractive(hold, frame, getScale) {
      ...
      if (resizable && !hold.querySelector('.resize_handle')) {
        ...
        handle.addEventListener('pointerdown', (e) => {
          ...
          e.preventDefault();
          e.stopPropagation();
          startX = e.clientX / getScale();
          startY = e.clientY / getScale();
          startW = hold.offsetWidth;
          startH = hold.offsetHeight;
          const img = hold.querySelector('img');
          hold._aspectRatio = img?.naturalWidth && img?.naturalHeight ? img.naturalWidth / img.naturalHeight : null;
          live = true;
          handle.setPointerCapture(e.pointerId);
        });
    ...
    function clampHoldInFrame(x, y, hold, frame) {
      return [Math.max(0, Math.min(frame.clientWidth - hold.offsetWidth, x)), Math.max(0, Math.min(frame.clientHeight - hold.offsetHeight, y))];
    }
    ...
    function bindTrack(video, el, track) {
      if (!el || !track) return;
      const update = () => {
        const on = video.currentTime >= (track.start ?? 0) && video.currentTime <= (track.end ?? Infinity);
    ...
    function bindControls(item, gpVideo) {
      ...
      const tick = () => {
        ...
        if (progressEl && isFinite(gpVideo.duration) && gpVideo.duration > 0) progressEl.style.width = `${(gpVideo.currentTime / gpVideo.duration) * 100}%`;
      };
      ...
      if (trackEl) {
        ...
        const seek = (cx) => {
          ...
          if (isFinite(gpVideo.duration) && gpVideo.duration > 0) gpVideo.currentTime = pct * gpVideo.duration;
        };
      ...
      gpVideo.addEventListener('ended', () => {
        item.querySelector('#play_button')?.classList.remove('is-playing');
        tick();
      });
    }
    ...
    function initFacecamCanvas(item, gpVideo, fcHold, x1, y1, x2, y2) {
      if (item._fcRafId) {
        cancelAnimationFrame(item._fcRafId);
        item._fcRafId = null;
      }
      ...
      function resizeFcCanvas() {
        const w = fcHold.clientWidth,
          h = fcHold.clientHeight;
      ...
      function drawFrame() {
        const vw = gpVideo.videoWidth,
          vh = gpVideo.videoHeight;
        const cw = fcHold.clientWidth,
          ch = fcHold.clientHeight;
      ...
      function rafLoop() {
        drawFrame();
        item._fcRafId = requestAnimationFrame(rafLoop);
      }
      function startLoop() {
        if (!item._fcRafId) rafLoop();
      }
      function stopLoop() {
        if (item._fcRafId) {
          cancelAnimationFrame(item._fcRafId);
    ...
    function applyMode(mode, refs) {
      const { frame, gpHold, gpVideo, fcHold, chatHold, splitHandle, hasFacecam, hasChatSplit, bindFcDrag, unbindFcDrag, applyGpZoom, gpZoom, applySplit, getSplitPct } = refs;

      // Allow split if facecam OR chat available — overlay always allowed
      ...
      // ── fcHold visibility + chat split logic ─────────────────────────────────
      if (fcHold) {
        ...
        if (enteringChatSplit && !frame._chatSplitActive) {
          // Enter chat split — show fcHold, inject chat video, hide chat overlay
          frame._chatSplitActive = true;
          fcHold.style.display = '';

          // Store current chat overlay visibility and hide it
          if (chatHold) {
            frame._chatOverlayWasVisible = chatHold.style.visibility !== 'hidden';
            chatHold.style.visibility = 'hidden';
          }

          // Inject chat video into fcHold if not already there
          ...
          // Leave chat split — remove video, restore chat overlay
      ...
      requestAnimationFrame(() => {
        if (safeMode === 'is-overlay' || safeMode === 'is-full') {
          const hw = gpHold.clientWidth,
            hh = gpHold.clientHeight;
    ...
    function buildPanelAPI(refs) {
      const { frame, gpHold, gpVideo, fcHold, chatHold, titleHold, titleText, subHold, textGroup, imgHold, imgEl, getGpZoom, applyGpZoom, applyModeFn, getSplitPct } = refs;

      // ── Image state — closed-over, same pattern as gpZoom ───────────────────
      let imgScale = 0;
      let imgX = 0.5;
      let imgY = 0.3;
      ...
      function applyImgPosition(x, y) {
        ...
        const hw = imgHold.offsetWidth,
          hh = imgHold.offsetHeight;
      ...
      window.canvasAPI = {
        ...
        getMode() {
          return frame._currentMode ?? 'is-full';
        },

        setTitle(str) {
          renderTitlePills(titleHold, str);
        },
        setTitleVisible(bool) {
          if (titleHold) titleHold.style.visibility = bool ? '' : 'hidden';
        },
        setSubtitleVisible(bool) {
          if (subHold) subHold.style.visibility = bool ? '' : 'hidden';
        },
        setChatVisible(bool) {
          if (chatHold) chatHold.style.visibility = bool ? '' : 'hidden';
        },
        setChatBlend(mode) {
        ...
        setTitleZone(zone) {
          applyTextZones(textGroup, zone, textGroup?._subZone ?? 'bottom');
        },
        setSubtitleZone(zone) {
          applyTextZones(textGroup, textGroup?._titleZone ?? 'top', zone);
        },
        ...
        setImagePosition(x, y) {
          applyImgPosition(x, y);
        },
        setImageScale(scale) {
          applyImgScale(scale);
        },

        setImageVisible(bool) {
          if (!imgHold) return;
          imgHold.style.visibility = bool ? '' : 'hidden';
        },

        setFacecamVisible(bool) {
          if (fcHold) fcHold.style.display = bool ? '' : 'none';
        },
        ...
        seekTo(secs) {
          if (gpVideo) gpVideo.currentTime = secs;
        },
        ...
        getState() {
          ...
          return {
            ...
            title:
              Array.from(titleHold?.querySelectorAll('.title_text') ?? [])
                .map((el) => el.textContent.trim())
                .join(' ') ||
              document.querySelector('[wized="stream_clip_title-list"]')?.textContent.trim() ||
              '',
    ...
    function initCanvas(item, opts = {}) {
      ...
      function applyGpZoom(zoom) {
        ...
        const coverScale = Math.max(hw / vw, hh / vh);
        const zoomMin = frame._currentMode === 'is-split' ? 1 : Math.max(0.2, hw / vw / coverScale);
      ...
      if (gpHold && gpVideo) {
        ...
        makeDraggable(gpHold, {
          ...
          clamp: (x, y) => {
            const hw = gpHold.clientWidth,
              hh = gpHold.clientHeight;
            const dw = gpVideo.offsetWidth,
              dh = gpVideo.offsetHeight;
      ...
      const tracks = {
        ...
        title: { start: 0, end: null }, // null = until clip end
      ...
      gpVideo.addEventListener('timeupdate', () => {
        ...
        } else if (gpVideo.currentTime >= effectiveEnd) {
          gpVideo.pause();
          gpVideo.currentTime = effectiveStart;
        }
      });
      ...
      function bindFcDrag() {
        ...
        function onResizeDown(e) {
          ...
          startX = e.clientX / getScale();
          startY = e.clientY / getScale();
          startW = fcHold.offsetWidth;
          startH = fcHold.offsetHeight;
          resizeLive = true;
          handle.setPointerCapture(e.pointerId);
        }
    ...
    function reapplyCrop(item) {
      ...
      if (chatHold && gpVideo && !chatHold._videoInit) {
        ...
        if (chatUrl) {
          ...
          // Mark that this item has a chat URL — enables split mode without facecam
      ...
      if (cropValid) {
        ...
        // No facecam — if chat exists, allow split; otherwise full
        if (wasValid !== false) {
    ...
    function bootCanvas() {
      ...
      if (item) {
        ...
        reapplyCrop(item);
        // Re-sync panel after reapplyCrop may have changed mode
