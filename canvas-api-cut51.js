(function () {
  ...
  function buildPanelAPI(refs) {
    ...
    window.canvasAPI = {
      setMode(mode)            { _setModeRouted(mode); },
      ...
      function _applyReframe(srcSec, ot, clip) {
        ...
        if (clip !== undefined) {
          ...
          if (cid !== _lastLayoutClipId) {
            ...
            // LAYOUT: nearest upstream layout override (cascade) → apply it; else base layout for the
            // current mode (when mode changed or we're leaving an inherited override).
            const L = _cascadeOverride(tracks.video.clips, clip, 'layout');
