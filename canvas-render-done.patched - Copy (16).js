(function () {
  ...
  const _RENDER_SCRIPT_BUILD = 'pb-20260502a-ios-remux-bg';
  ...
  async function extractRawMp4Audio(sourceUrl, startSec, endSec) {
    const MP4Box = _mbMp4?.default ?? _mbMp4;
    if (!MP4Box) return null;

    const res = await fetch(sourceUrl);
    if (!res.ok) throw new Error(`extractRawMp4Audio: fetch ${res.status}`);
    ...
    return new Promise((resolve, reject) => {
      ...
      file.onFlush = () => {
        if (!track || !samples.length) { resolve(null); return; }
        resolve({ track, desc, samples });
      };

      file.onError = e => reject(new Error(String(e)));
      file.appendBuffer(srcBuf);
      file.flush();
    });
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      async function finishMp4() {
        try {
          ...
          // Wait for background audio extraction to finish (started in setupMp4AudioSource).
          // In most cases it completes long before the render loop finishes.
          if (_rawAudioPromise) { await _rawAudioPromise; _rawAudioPromise = null; }
