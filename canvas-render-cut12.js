  ...
  if (bR) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const _onSeeked = async function onSeeked() {
        ...
        // t = SOURCE time (subtitles are source-keyed); tlT = TIMELINE/output time (title & image
        // visibility windows are set in timeline space → must follow cuts/reorder). For non-NLE the
        // two coincide so callers pass t for both.
        function drawCanvasFrame(t, tlT = t) {
          ...
          if (imgLayout && state.imageVisible) {
            ...
            const imgEnd   = tracks.image?.end   ?? (isNLE ? totalDuration : duration);
            if (tlT >= imgStart && tlT <= imgEnd) {   // timeline-time window
...

          {
            ...
            if (titleLines.length && state.titleVisible) {
              ...
              const titleEnd   = tracks.title?.end   ?? (isNLE ? totalDuration : duration);
              if (tlT >= titleStart && tlT <= titleEnd) renderTitleAtY(ctx, titleLines, titleY, ts);   // timeline-time window
            }
        ...
        async function emitFrameAt(index) {
          ...
          if (_inCut) {
            ...
            drawCanvasFrame(t, isNLE ? (index / FPS) : t);   // NLE: index/FPS = output time for title/image windows
          }
        ...
        if (_useEngine) {
          ...
          const _engTick = () => {
            ...
            drawCanvasFrame(isFinite(st) ? st : 0, ot);      // composite engine frame + overlays onto rc (ot = timeline time for title/image)
