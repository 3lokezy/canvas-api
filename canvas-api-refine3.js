(function () {
  ...
  // ── Clip GRADE (always-on look / mild copyright-fingerprint filter) ─────────────
  // A constant colour grade + light sharpen applied to the WHOLE frame on BOTH surfaces:
  //   • live  — CSS filter on the frame element, folded into _bindFx (composes with the
  //             desaturate FX: saturate() chains multiply, so grade × desat just works)
  //   • render— ctx.filter self-redraw in drawCanvasFrame (reads getGradeFilter — same source)
  // ONE backend knob: tweak the numbers or flip `enabled`. Values tuned in grade-lab.html.
  // Sharpen is a 3×3 unsharp convolution via the injected SVG #clipSharpen (identity at 0);
  // it's the only part that needs feature-detection on the render canvas (ctx.filter url()).
  const CLIP_GRADE = { enabled: true, saturate: 1.4, contrast: 1.14, brightness: 1.01, sharpen: 0.4 };
  ...
  // The grade filter string. withSharpen=false yields the taint-safe colour-only part (render
  // appends the url() sharpen separately, gated on canvas url()-filter support).
  function _gradeFilter(withSharpen) {
    if (!CLIP_GRADE.enabled) return '';
  ...
  function _bindFx(video, frameEl) {
    if (!frameEl) return;
    _ensureSharpenSVG();                        // grade sharpen ref for live + render
    ...
    const apply = (ot) => {
      ...
      // Constant grade is the BASE filter (always on); a desaturate moment appends its
      // saturate() on top — chained saturate() multiplies, so grade × desat composes cleanly.
      const base = _gradeFilter(true);
      const fil  = fx.desat > 0 ? (base ? base + ' ' : '') + `saturate(${(1 - fx.desat).toFixed(3)})` : base;
  ...
  function buildPanelAPI(refs) {
    ...
    window.canvasAPI = {
      ...
      // Clip grade — the constant look filter. Render reads this (same source as the live
      // preview) so export == preview. withSharpen=false = colour-only (render appends the
      // url() sharpen itself, gated on canvas url()-filter support). getClipGrade for tuning.
