...
if (kickLogo && barEl) {
  ...
  function renderSubWordAtY(ctx, text, y, ts, anim) {
    const emph   = !!(window._capIsEmph && window._capIsEmph(text));   // keyword/shouted → colour + scale pop
    const eScale = emph ? (window._capEmphCfg?.scale || 1.14) : 1;
  ...
  function renderSubChunkAtY(ctx, chunk, activeIdx, y, ts, anim) {
    ...
    for (let i = 0; i < metrics.length; i++) {
      ...
      const emph     = !!(window._capIsEmph && window._capIsEmph(m.text));   // colour-only in chunk mode (scale would shift the row layout)
