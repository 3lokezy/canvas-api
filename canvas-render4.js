(function () {
  ...
  async function renderComposition(onProgress) {
    ...
    if (useMp4) {
      ...
      const startTimeoutMs = _isIOS ? 6000 : 5000;
      ...
      startMp4Output = async () => {
        ...
        if (audioSource && mp4AudioBuffer) {
          await audioSource.add(mp4AudioBuffer);
        }
        outputStarted = true;
    ...
    return new Promise((resolve, reject) => {
      ...
      rv.addEventListener('seeked', async function onSeeked() {
        ...
        try {
          await rv.play();
          console.log('Render: play resolved, unmuting — paused:', rv.paused, 'time:', rv.currentTime.toFixed(3));
          rv.muted  = false;
          rv.volume = 1;
          if (audioCtx?.state === 'suspended') {
            await Promise.race([audioCtx.resume(), new Promise(r => setTimeout(r, 500))]);
