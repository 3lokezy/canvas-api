(function () {
    ...
    function initCanvas(item, opts = {}) {
      ...
      // ── Passive mode — apply clean full layout and return ────────────────────
      if (!interactive) {
        MODES.forEach(m => {
          frame.classList.remove(m);
          gpHold?.classList.remove(m);
        ...
        if (fcHold) fcHold.style.display = 'none';
        // Force correct video sizing and aspect ratio in passive mode
      ...
      if (gpHold && gpVideo) {
        ...
        gpVideo.addEventListener('loadedmetadata', () => {
            createBgVideo();
            // Only reset zoom if restore hasn't set it already
            if (!frame._zoomRestored) applyGpZoom(gpZoom);
            const { effectiveStart } = getTrackBounds();
    ...
    function reapplyCrop(item) {
      ...
      if (chatHold && gpVideo && !chatHold._videoInit) {
        ...
        // ── Passive mode — force first frame render ───────────────────────────────
      ...
      if (cropValid) {
        ...
        if (item._applyMode) {
          ...
          // Passive mode — apply split classes directly
          MODES.forEach(m => {
            clipFrame?.classList.remove(m);
            gpHold?.classList.remove(m);
            fcHold.classList.remove(m);
        ...
        if (item._applyMode) {
          ...
          // Passive mode — apply full classes directly
          MODES.forEach(m => {
            clipFrame?.classList.remove(m);
            gpHold?.classList.remove(m);
            fcHold.classList.remove(m);
        ...
        // Apply Supabase canvas state to passive render
    ...
    function bootCanvas() {
      ...
      // Passive clips — initCanvas once, reapplyCrop every boot for fresh Wized data
