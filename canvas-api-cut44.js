  ...
  function initCanvas(item, opts = {}) {
    ...
    if (gpHold && gpVideo) {
      ...
      function _applyReframe(srcSec, ot, clip) {
        ...
        // (1) PER-CLIP LAYOUT — re-apply the active clip's effective layout (override ?? base
        //     modeStates) on each clip change (engine passes `clip`; the engine-off fallback omits it).
        if (clip !== undefined) {
          const cid = clip ? clip.id : null;
          if (cid !== _lastLayoutClipId) {
            _lastLayoutClipId = cid;
            const L = clip && clip.override && clip.override.layout;
            if (L) { applyLayout(L); _lastLayoutWasOverride = true; }            // clip broke away → its layout
            else if (_lastLayoutWasOverride) { applyLayout(modeStates[frame._currentMode]); _lastLayoutWasOverride = false; }  // leaving an override → restore base
            // else: no override now or before → leave geometry untouched (zero change when unused)
          }
        }
