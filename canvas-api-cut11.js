(function () {
  ...
  function initFacecamCanvas(item, gpVideo, fcHold, x1, y1, x2, y2) {
    ...
    function pickFrameSource() {
      // Normal playback: gpVideo has a current frame — use it.
      if (!gpVideo.seeking && gpVideo.readyState >= 2) return gpVideo;
      // Transition seek: gpVideo briefly has no displayable frame (readyState<2 while the decode
      // pipeline resets, even when fully buffered). The double-buffer standby is pre-seeked to the
      // INCOMING clip and, while it's actively covering the gameplay panel (opacity:1), is showing
      // exactly the frame this facecam should switch to. Drawing the standby here keeps the facecam
      // in lockstep with the gameplay panel instead of holding the OLD clip's frame — which on a
      // reordered (real-seek) cut reads as a flash (new gameplay beside an old facecam, then snap).
      // It shares gpVideo's src, so the same fractional crop coords apply.
      const sb = window.canvasAPI?.getStandbyVideo?.();
      if (sb && sb.style.opacity === '1' && !sb.seeking && sb.readyState >= 2 && sb.videoWidth) return sb;
      // Neither paintable: hold the last good frame (don't clear).
      return null;
    }

    function drawFrame() {
      const src = pickFrameSource();
      if (!src) return;
      ...
      ctx.drawImage(src, srcX, srcY, srcW, srcH, (cw - dw) / 2, (ch - dh) / 2, dw, dh);
    }
  ...
  function buildPanelAPI(refs) {
    ...
    if (_wmVideoEl) {
      ...
      _wmVideoEl.addEventListener('play', () => {
        if (gpVideo.paused) _wmVideoEl.pause();
      });
    }
    ...
    function _syncMusicToVideo() {
      ...
      const inWindow   = t >= trackStart && t <= trackEnd;
      if (!inWindow || _musicMuted) {
        _musicAudio.pause();
        return;
      }
      const targetTime = _musicOffset + (t - trackStart);
      const drift      = Math.abs(_musicAudio.currentTime - targetTime);
      if (drift > 0.2) _musicAudio.currentTime = targetTime;
      if (_musicAudio.paused && !gpVideo.paused) _musicAudio.play().catch(() => {});
    }
