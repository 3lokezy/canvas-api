(function () {
  ...
  async function ensureMp4Support() {
    ...
    // but VideoEncoder + Mediabunny can still mux MP4 (AAC skipped in setupMp4AudioSource when missing).
    ...
    if (_canMP4) {
      if (_useBufferedMp4Audio && typeof AudioEncoder === 'undefined') {
        console.log('Render: MP4 output enabled (H.264); AudioEncoder missing — AAC skipped (video-only MP4)');
      } else {
      ...
      setupMp4AudioSource = async () => {
        ...
        if (_useBufferedMp4Audio) {
          if (typeof AudioEncoder === 'undefined') {
            console.warn('Render: AudioEncoder unavailable — skipping AAC (video-only MP4)');
            return;
          }
