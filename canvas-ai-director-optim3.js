(function () {
  ...
  const VERSION = 'v2.3-sep-guard-widefix';
  ...
  const CFG = (window.aiDirectorCfg = Object.assign({
    ...
    SCAN_FPS:  3,           // scene-scan sample rate — also the resolution of scan-derived tracks (was 2)
  ...
  async function scan() {
    ...
    if ((!window._scan || !window._scan.thumbs) && window.wcTracker?.scan) {
      try { window._scan = await window.wcTracker.scan({ fps: CFG.SCAN_FPS, thumbs: true }); } catch (_) {}
    }
  ...
  async function buildDigest() {
    ...
    return {
      ...
      gap_frames:      fr.frames,                                   // the JPEGs themselves (n8n → Gemini inline parts)
  ...
  function bottomBoxFor(s, absFrom, absTo) {
    ...
    const pick = (p) => {
      ...
      return scaleBox(personBox({ cx: pos.cx, cy: pos.cy, size: pos.size || p.size }), s.bottomZoom || 1);
    };
    ...
    if (s.bottomZoom && s.bottomZoom !== 1) return scaleBox(baseCamBox(), s.bottomZoom);
    return null;
  }
      ...
      if (s.mode === 'is-split') {
        try {
          ...
          if (!s.person) {
            ...
            const pos = sp && personPosIn(sp, off + s.from, off + s.to);
            if (sp && pos) {
              sEff = { ...s, person: sp };
          ...
          const box = bottomBoxFor(sEff, off + s.from, off + s.to);
          // SEPARABILITY GUARANTEE — the two panels must show DISJOINT subjects (the user's invariant:
          // split coordinates may never overlap). For person-based splits: ① the featured face may not sit
          // inside the bottom crop; ② the bottom subject may not sit inside the top punch's visible span.
          // Either overlap = the same face twice = no split.
          let clash = null;
          if (box && sEff.person) {
            ...
            if (featPos && featPos.cx > box.x1 - 0.02 && featPos.cx < box.x2 + 0.02 &&
              ...
              // a WIDE top shows the whole frame — it inevitably contains the bottom subject. This was the
              // gate the same-face screenshot slipped through (zoom 1 skipped the span check entirely).
              clash = 'top frame is wide — it contains the bottom subject';
            } else {
              const bcx = (box.x1 + box.x2) / 2, bcy = (box.y1 + box.y2) / 2;
              const span = (0.5 / lay.zoom) * 0.9;               // the top punch shows ~1/zoom of the frame
              if (Math.abs(bcx - lay.focusX) < span && Math.abs(bcy - lay.focusY) < span)
                clash = 'bottom subject inside top frame';
            }
          }
          if (box && !clash) {
            ...
            console.log('[ai-polish] split bottom ' + s.from.toFixed(1) + 's:',
              ...
              sEff.bottomZoom ? 'z' + sEff.bottomZoom : '', box);
          } else if (!baseCamBox()) {
            // No separable second subject AND no real cam to ride: FORCE FULL ON THE TARGET — featured-person
            // punch when validate framed one, honest wide otherwise. A split showing one face twice never ships.
            c.override.mode = 'is-full';
            console.log('[ai-polish] split→full ' + s.from.toFixed(1) + 's — ' + (clash || 'no distinct 2nd subject on screen'));
          } else if (clash) {
            // A real corner cam exists — ride the base cam bottom instead of the clashing person crop.
            console.log('[ai-polish] split ' + s.from.toFixed(1) + 's — ' + clash + '; riding base cam');
          }
