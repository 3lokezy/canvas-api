(function () {
  ...
  // Chrome iOS reports CriOS in UA. MediaRecorder video/mp4 silently produces empty
  // output on Chrome iOS, so we force the Mediabunny path there. Safari iOS lacks
  // AudioEncoder too but its MediaRecorder video/mp4 works correctly with audio.
  ...
  const _RENDER_SCRIPT_BUILD = 'pb-20260505r-timer-diag';
  ...
  // Chrome iOS lacks AudioEncoder → renders are silent. Expose a Wized flag
  // so a "use Safari for audio" nudge can be shown in the UI.
  // Auto-clears if Apple eventually ships AudioEncoder in WebKit.
  ...
  async function ensureMp4Support() {
    ...
    if (_canMP4) {
      if (_useBufferedMp4Audio && typeof AudioEncoder === 'undefined') {
        // All iOS without AudioEncoder (Chrome iOS + Safari iOS < 16.4) → video-only Mediabunny MP4.
        // Safari iOS previously fell back to MediaRecorder which produced VFR → TikTok rejected.
        console.log('Render: WebCodecs H.264 available; no AudioEncoder —',
          _isChromeIOS ? 'video-only MP4 (Chrome iOS)' : 'video-only MP4 (Safari iOS, no AudioEncoder — CFR for TikTok)');
      } else {
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  // MP4 AUDIO REMUX — iOS devices without AudioEncoder
  // Extracts raw AAC samples from the source MP4 and injects them into the
  // Mediabunny video-only output after finalize. No re-encoding needed.
  // ─────────────────────────────────────────────────────────────────────────────

  // Wraps a buffer so mp4box can accept it (needs an ArrayBuffer with .fileStart).
  ...
  // Fetch source video, parse with mp4box, return trimmed raw AAC samples.
  // Fetch runs outside the Promise constructor — errors propagate naturally.
  // Only the synchronous mp4box callbacks live inside new Promise() to avoid
  // the async-executor trap where thrown errors silently hang the Promise.
  async function extractRawMp4Audio(sourceUrl, startSec, endSec, signal) {
    // Lazy-load mp4box only when actually needed (iOS no-AudioEncoder path).
    // Importing it in ensureMp4Support alongside Mediabunny interferes with
    // the Mediabunny encoder on non-iOS platforms.
    if (!_mbMp4) {
    ...
    if (typeof MP4Box?.createFile !== 'function') {
      console.warn('Render: mp4box.createFile not found — exports:', Object.keys(_mbMp4 || {}));
      return null;
    }
    ...
    if (signal?.aborted) return null;
    console.log('Render: source audio fetched —', (srcBuf.byteLength / 1048576).toFixed(1), 'MB, parsing…');
    ...
    return new Promise((resolve, reject) => {
      ...
      file.onSamples = (id, _, batch) => {
        ...
        // mp4box@0.5.2 onFlush is unreliable — resolve via sample count instead.
        if (track.nb_samples && totalReceived >= track.nb_samples) {
          settle(samples.length ? { track, desc, samples } : null);
        }
      };

      // Fallback in case nb_samples is 0 or not set.
  ...
  // Parse video-only Mediabunny output and rebuild it with an added audio track.
  async function remuxWithAudio(videoBuffer, audioData) {
    ...
    const { vTrack, vDesc, vSamples } = await new Promise((resolve, reject) => {
      ...
      const vSamples = [];
      let vSettled = false;
      let vTotalReceived = 0;

      const vSettle = result => { if (!vSettled) { vSettled = true; resolve(result); } };
      const vFail   = err   => { if (!vSettled) { vSettled = true; reject(err); } };

      file.onReady = info => {
        vTrack = info.videoTracks?.[0];
        if (!vTrack) { vFail(new Error('remuxWithAudio: no video track in Mediabunny output')); return; }
        try {
          vDesc = file.getTrackById(vTrack.id).mdia.minf.stbl.stsd.entries[0];
        } catch (_) {}
      ...
      file.onSamples = (id, _, batch) => {
        vSamples.push(...batch);
        vTotalReceived += batch.length;
        if (vTrack?.nb_samples && vTotalReceived >= vTrack.nb_samples) {
    ...
    // addTrack's `description` is a child box added INSIDE the sample entry.
    // For avc1 the child must be the avcC config box, NOT the avc1 entry itself
    // (passing vDesc would nest avc1 inside avc1 — no SPS/PPS → no video decode).
    // For mp4a the child must be the esds box, not the mp4a entry.
    ...
    for (const s of vSamples) {
      outFile.addSample(videoId, s.data, {
        duration: s.duration, dts: s.dts, cts: s.cts ?? s.dts,
        is_sync: s.is_sync,
      });
    }
  ...
  async function renderComposition(onProgress) {
    ...
    // All iOS without AudioEncoder (both Chrome iOS and Safari iOS < 16.4) → Mediabunny video-only.
    // Previously Safari iOS fell to MediaRecorder which has audio but produces VFR MP4 → TikTok
    // frame_rate_check_failed. Silent CFR MP4 is better than audio+VFR for posting to TikTok.
    // MediaRecorder fallback only fires on non-iOS platforms genuinely lacking VideoEncoder.
    const useMp4 = webCodecsMp4Ok &&
      !(_useBufferedMp4Audio && typeof AudioEncoder === 'undefined' && !_isChromeIOS && !_isIOS);
    const _noAudioIOS = _isIOS && typeof AudioEncoder === 'undefined';
    console.log('Render: [3] encoding setup — useMp4:', useMp4,
      _isIOS ? '(iOS)' : '',
      _isChromeIOS ? '(Chrome iOS — video-only)' : _noAudioIOS ? '(Safari iOS no AudioEncoder — video-only CFR)' : '',
    ...
    if (useMp4) {
      ...
      setupMp4AudioSource = async () => {
        ...
        if (typeof AudioEncoder === 'undefined') {
          // No AudioEncoder (Chrome iOS always; Safari iOS ≤ 16.3).
          // Fire raw AAC extraction in the background — do NOT await here.
          // Awaiting would block onSeeked before startMp4Output runs, keeping
          // progress at 0% for the entire source-video fetch (100–300 MB).
          // finishMp4 awaits _rawAudioPromise before remuxing.
          ...
          return; // return immediately so the render loop can start
        }
        if (audioCtx.state === 'suspended') {
          await Promise.race([audioCtx.resume(), new Promise(r => setTimeout(r, 500))]);
        }
        try {
          ...
          // Pass the ArrayBuffer directly — decodeAudioData detaches it (zeroes byteLength) so
          // the browser can free the raw bytes as soon as decoding starts. Do NOT .slice(0) here;
          // slicing creates a full second copy (~100-300 MB on a typical source clip) that stays
          // alive alongside the decoded PCM and causes OOM crashes on mobile.
          let rawBytes = await audioRes.arrayBuffer();
          let fullPcm = await audioCtx.decodeAudioData(rawBytes); // rawBytes detached → 0 bytes now
          rawBytes = null; // explicit hint — detached buffers are already 0 B but clear the ref
          mp4AudioBuffer = trimAudioBuffer(audioCtx, fullPcm, effectiveStart, effectiveEnd);
          fullPcm = null; // full PCM no longer needed; only the trimmed slice is kept
          audioSource = new MBAudioBufferSource({
            codec: 'aac',
            bitrate: 128_000,
          });
        } catch (e) {
          console.warn('Render: audio fetch/decode failed (CORS or network) — exporting video-only:', e.message);
          audioSource = null;
        }
      };
      ...
      startMp4Output = async () => {
        ...
        output = new Output({
          // 'false' writes moov at the end of the file (appended at finalize) rather than
          // holding everything in a temporary buffer to reorder moov to front ('in-memory').
          // This avoids the finalize-time 2× memory spike while keeping standard MP4 layout
          // that AudioBufferSource can properly mux. TikTok doesn't require fast-start.
    ...
    return new Promise((resolve, reject) => {
      ...
      async function finishMp4() {
        ...
        try {
          if (!outputStarted || !videoSource || !output) {
            console.warn('Render: finishMp4 guard failed', { outputStarted, hasVS: !!videoSource, hasOut: !!output });
            throw new Error('MP4 output was not started');
          }
          console.log('Render: closing videoSource…');
          try { videoSource.close(); } catch (e) { console.error('Render: videoSource.close() threw:', e); throw e; }
          console.log('Render: videoSource closed, closing audioSource…');
          try { if (audioSource) audioSource.close(); } catch (e) { console.error('Render: audioSource.close() threw:', e); throw e; }
          console.log('Render: audioSource closed');
          mp4AudioBuffer = null;
          console.log('Render: calling output.finalize()…');
          await output.finalize();
          console.log('Render: finalize complete, buffer', (output.target.buffer?.byteLength ?? 0) / 1048576 | 0, 'MB');

          // Wait for background audio extraction (started in setupMp4AudioSource).
          // If the source fetch is still in-flight, abort it after 20 s and fall
          // back to video-only — prevents hanging indefinitely on slow connections
          // or when the source MP4 has moov at the end.
          if (_rawAudioPromise) {
            // Race extraction against a 20 s timeout. Aborting the controller
            // signals the fetch but doesn't resolve the Promise — we need the
            // race so finalize can continue even if extraction hangs.
            await Promise.race([
              _rawAudioPromise,
              new Promise(r => setTimeout(() => {
                console.warn('Render: audio extraction timeout — falling back to video-only');
                _extractAbort?.abort();
                r();
              }, 20_000)),
            ]);
          ...
          if (rawAudioData && _mbMp4) {
            try {
              console.log('Render: remuxing', rawAudioData.samples.length, 'raw AAC samples into video output…');
              finalBuffer = await remuxWithAudio(finalBuffer, rawAudioData);
              console.log('Render: remux complete —', (finalBuffer.byteLength / 1048576).toFixed(1), 'MB');
            } catch (e) {
              console.warn('Render: audio remux failed, falling back to video-only:', e);
            }
            rawAudioData = null;
          }
          ...
          cleanup();
          resolve({
            video: new Blob([finalBuffer], { type: 'video/mp4' }),
            thumb: thumbJpegBlob,
          });
      ...
      rv.addEventListener('seeked', async function onSeeked() {
        ...
        // ── Render one composited frame at logical time `t` ───────────────
        //  `t` is the deterministic source-time we declare for this output
        //  frame (`effectiveStart + index/FPS`). Visual content reads from
        //  rv's currently-presented frame, so capture is paced by rVFC.
        ...
        // ── Frame-accurate CFR capture ────────────────────────────────────
        //  Output frames are emitted at deterministic timestamps `index/FPS`,
        //  paced by `requestVideoFrameCallback` (preferred) or setInterval
        //  (fallback). Frame visuals come from rv's current state at each
        //  callback; if the source FPS differs from output FPS, frames are
        //  duplicated or skipped to keep the output strict 30fps CFR.
        const totalFrames = Math.max(1, Math.round(totalDuration * FPS));
        let nextEmitIndex = 0;
        let renderFinished = false;
        // iOS: seek frame-by-frame instead of play/pause per frame.
        // play() after a programmatic pause is unreliable on iOS Safari —
        // it stalls or fails silently, rVFC never re-arms, and finalizeRender
        // pads all remaining frames from the last canvas position (= duplicate
        // frames, bad quality, audio desync, 10-minute renders).
        const _iosSeekMode = _isIOS && useMp4;
        ...
        async function emitFrameAt(index) {
          ...
          if (useMp4) {
            // In seek mode (iOS) rv is already paused and positioned by the seek loop —
            // no play/pause cycle needed. On other platforms, pause before the encode
            // await so rv doesn't race ahead and cause rVFC catch-up duplicate frames.
            if (!_iosSeekMode) {
              rv.pause();
              if (chatVid) chatVid.pause();
            }
            const _isLastFrame = (index + 1) >= totalFrames;
            if (index < 3) console.log('Render: [dbg] emitting frame', index, '/', totalFrames, 'last:', _isLastFrame);
            if (_isLastFrame) {
              // Last frame: do NOT await — Mediabunny backpressure only resolves when
              // the encoder is ready for the NEXT frame. For the final frame there is
              // no next frame, so awaiting deadlocks. videoSource.close() in finishMp4
              // flushes the encoder and this frame is encoded as part of that flush.
              videoSource.add(index / FPS, 1 / FPS);
              // rv stays paused — finalizeRender → finishMp4 handles cleanup.
            } else {
              await videoSource.add(index / FPS, 1 / FPS);
              if (index < 3) console.log('Render: [dbg] encoder ack frame', index);
              if (!_iosSeekMode && !rv.ended) {
                const ctBefore = rv.currentTime;
                try { await rv.play(); } catch (playErr) { console.warn('Render: [dbg] rv.play() threw after frame', index, playErr?.message); }
                if (index < 10) console.log(`Render: [dbg] after play() frame=${index} ct=${rv.currentTime.toFixed(4)} paused=${rv.paused} (was ${ctBefore.toFixed(4)})`);
                if (chatVid && !chatVid.ended) { try { await chatVid.play(); } catch (_) {} }
              }
            }
          }

          const progress = (index + 1) / totalFrames;
          onProgress?.(Math.min(1, Math.max(0, progress)));
        }

        async function pumpEmits(currentMediaTime) {
          const offset = currentMediaTime - effectiveStart;
          if (nextEmitIndex < 5) console.log(`Render: [pump] called ct=${currentMediaTime.toFixed(4)} offset=${offset.toFixed(4)} nextEmit=${nextEmitIndex} threshold=${(nextEmitIndex/FPS).toFixed(4)}`);
        ...
        async function finalizeRender() {
          ...
          renderFinished = true;
          console.log('Render: finalizeRender entered, nextEmitIndex:', nextEmitIndex, '/', totalFrames);
          ...
          // Pad any remaining frames so output has exactly totalFrames at 1/FPS each.
          // Necessary if rv ended early or rVFC stopped firing before reaching effectiveEnd.
        ...
        // Track whether a pumpEmits is already in flight on the timer path
        // to prevent concurrent calls when encoder backpressure delays a tick.
        let _pumpRunning = false;

        const _hasVFC = typeof rv.requestVideoFrameCallback === 'function';

        if (_iosSeekMode) {
          // iOS: seek rv to each frame's exact timestamp instead of play/pause cycling.
          // play() after a programmatic pause is unreliable on iOS Safari — it stalls or
          // fails silently, rVFC never re-arms, and remaining frames are padded from the
          // same canvas position, producing duplicates and audio desync.
          rv.pause();
          if (chatVid) chatVid.pause();
          console.log('Render: [iOS] seek-based loop — frame-by-frame seek, totalFrames:', totalFrames);
          (async () => {
            try {
              for (let i = 0; i < totalFrames; i++) {
                const t = effectiveStart + i / FPS;
                // Only issue a seek if we're not already at this position.
                ...
                // Set chatVid time without awaiting — visual-only overlay, lag is acceptable.
          ...
          console.log('Render: using requestVideoFrameCallback (frame-accurate CFR)');

          // rVFC requires the video element to be visible to the compositor.
          // If it's inside a display:none / zero-dimension container (e.g. after a DOM
          // reshuffle), Chrome silently suppresses all callbacks. Detect this with a
          // 2-second watchdog and fall back to the timer loop automatically.
          let _lastPumpPromise = Promise.resolve();
          let _lastRvfcFiredAt = Date.now();
          let _rvfcEverFired = false;

          const startTimerFallback = () => {
            console.warn('Render: rVFC not firing (video may be hidden) — switching to timer loop');
            if (_rvfcHandle != null) {
              try { rv.cancelVideoFrameCallback(_rvfcHandle); } catch (_) {}
              _rvfcHandle = null;
            }
          ...
          // Watchdog: if rVFC hasn't fired within 2 s, bail to timer loop.
          const _rvfcWatchdog = setTimeout(() => {
            if (!_rvfcEverFired && !renderFinished) startTimerFallback();
          }, 2000);

          _rvfcGuardInterval = setInterval(async () => {
            if (renderFinished) { clearInterval(_rvfcGuardInterval); _rvfcGuardInterval = null; return; }
            // Safety net for full-length clips: rVFC stops when video ends naturally.
            // Only trigger if rVFC has been silent ≥ 500 ms AND video has ended.
            if (_rvfcEverFired && rv.ended && (Date.now() - _lastRvfcFiredAt) > 500) {
          ...
          const onVideoFrame = async (now, metadata) => {
            ...
            if (nextEmitIndex < 3) console.log('Render: [dbg] rVFC fired, mediaTime:', metadata.mediaTime.toFixed(3), 'nextEmit:', nextEmitIndex);
            await (_lastPumpPromise = pumpEmits(metadata.mediaTime));
            if (nextEmitIndex >= totalFrames || metadata.mediaTime >= effectiveEnd - 0.001 || rv.ended) {
              finalizeRender();
              return;
            }
            // Re-arm only after pumpEmits resolves — rVFC naturally paces
            // to the encoder's throughput, never building a backlog.
            _rvfcHandle = rv.requestVideoFrameCallback(onVideoFrame);
          };
          _rvfcHandle = rv.requestVideoFrameCallback(onVideoFrame);
        } else {
          console.warn('Render: rVFC unavailable — using timer loop');
          startTimerLoop();
        }
        ...
        function startTimerLoop() {
          ...
          const onTimerTick = () => {
            ...
            const paused = rv.paused;
            if (_timerTickCount <= 10 || _timerTickCount % 30 === 0) {
              console.log(`Render: [timer] tick=${_timerTickCount} ct=${ct.toFixed(4)} paused=${paused} pumpRunning=${_pumpRunning} nextEmit=${nextEmitIndex}/${totalFrames}`);
            }
            if (!frame._rendering || renderFinished || _pumpRunning) return;
            _pumpRunning = true;
            pumpEmits(rv.currentTime).then(() => {
              _pumpRunning = false;
              if (nextEmitIndex >= totalFrames || rv.currentTime >= effectiveEnd - 0.001 || rv.ended) {
                finalizeRender();
              }
            }).catch((e) => { console.error('Render: [timer] pumpEmits threw:', e); _pumpRunning = false; });
          };
          if (_useWorkerTimer) {
            _timerWorker.onmessage = onTimerTick;
            _timerWorker.postMessage('start');
          } else {
            _timerInterval = setInterval(onTimerTick, Math.round(1000 / FPS));
          }
        }
