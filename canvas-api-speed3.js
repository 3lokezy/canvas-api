(function () {
  ...
  const SOUNDBOARD_ANCHOR  = 'source';   // 'source' = a sound is PINNED TO THE FOOTAGE — it follows clip
                                         // reorder/delete/trim and drops out if its moment is cut. 'output' =
                                         // a fixed timeline position. Resolved live in getSoundboardCues; manual
                                         // placements convert output→source at the boundary (see _anchorStore).
  const IMAGE_ANCHOR       = 'source';   // same, for the multi-image (gif/meme) track — resolved in getImageCues.
  const SOUNDBOARD_GAIN    = 0.65;       // ⬅ THE knob. Universal master gain for ALL soundboard snippets, preview + export.
                                         //    0..1 = quieter (1 = raw), >1 amplifies (may clip). Edit → redeploy → hard reload.
  ...
  // Manual placements arrive in OUTPUT (timeline) space; when a track is footage-anchored ('source') we store
  // the SOURCE moment under the drop so the cue follows clip reorder/delete/trim like the AI-placed ones. A
  // dead-space drop (no clip under it) has no source moment → keep the raw value. AI/restore paths already
  // carry source time, so they bypass this (they write via the bulk setters, not add/move).
  const _anchorStore = (outSec, mode) => {
    const v = Math.max(0, +outSec || 0);
    if (mode !== 'source') return v;
    const src = window.canvasAPI?.outputTimeToSourceTime?.(v);
    return (src == null) ? v : src;
  };
  ...
  // ── Clip GRADE (look / mild copyright-fingerprint filter) — VIDEO LAYERS ONLY ────
  // A constant colour grade + light sharpen applied to the video picture (gameplay + its
  // blurred bg, facecam, chat) but NOT the overlays (gifs, title, subtitle, badges, watermark).
  //   • live  — a CSS rule scoped to .gameplay_hold/.facecam_hold/.chat_hold, gated by an
  //             `.is-graded` class on .clip_canvas (overlays are DOM siblings, so untouched).
  //   • render— ctx.filter self-redraw over the video region before overlays draw, + the one
  //             floating-chat draw (reads getGradeFilter — same numbers, so preview == export).
  // TWO knobs: CLIP_GRADE = the LOOK (tuned in grade-lab.html); _gradeEnabled = the on/off toggle
  // (per-clip, persisted in getState, wired to a [data-grade] button + undoable via history).
  // Sharpen is a 3×3 unsharp convolution via the injected SVG #clipSharpen (identity at 0).
  const CLIP_GRADE = { saturate: 1.25, contrast: 1.14, brightness: 0.96, sharpen: 0.2 };
  let _gradeEnabled = true;   // default-on; per-clip, overridden by restore/toggle

  // Inject the sharpen SVG once so both `filter: url(#clipSharpen)` (live) and ctx.filter (render)
  // can reference it. Parametric kernel: center = 1+4a, edges = −a, divisor 1 → no-op at a=0.
  function _ensureSharpenSVG() {
    if (!CLIP_GRADE.sharpen || document.getElementById('clipSharpen')) return;
    const a = CLIP_GRADE.sharpen, c = (1 + 4 * a).toFixed(3), e = (-a).toFixed(3);
    const NS = 'http://www.w3.org/2000/svg';
    const svg = document.createElementNS(NS, 'svg');
    svg.setAttribute('aria-hidden', 'true');
    svg.style.cssText = 'position:absolute;width:0;height:0;pointer-events:none';
    const f = document.createElementNS(NS, 'filter');
    f.setAttribute('id', 'clipSharpen');
    ...
    m.setAttribute('order', '3'); m.setAttribute('preserveAlpha', 'true'); m.setAttribute('edgeMode', 'duplicate');
    m.setAttribute('kernelMatrix', `0 ${e} 0 ${e} ${c} ${e} 0 ${e} 0`);
    f.appendChild(m); svg.appendChild(f);
    (document.body || document.documentElement).appendChild(svg);
  }
  // The raw grade filter string (ignores the toggle — the .is-graded class / render guard gate it).
  function _gradeFilterRaw(withSharpen) {
    const p = [`saturate(${CLIP_GRADE.saturate})`, `contrast(${CLIP_GRADE.contrast})`, `brightness(${CLIP_GRADE.brightness})`];
    if (withSharpen && CLIP_GRADE.sharpen > 0) p.push('url(#clipSharpen)');
    return p.join(' ');
  }
  // Toggle-gated string for the RENDER (returns '' when off, so render skips the pass entirely).
  function _gradeFilter(withSharpen) { return _gradeEnabled ? _gradeFilterRaw(withSharpen) : ''; }

  // Live surface: a stylesheet rule that grades ONLY the video holds (overlays are siblings).
  function _ensureGradeCSS() {
    _ensureSharpenSVG();
    if (document.getElementById('clip-grade-css')) return;
    const st = document.createElement('style');
    st.id = 'clip-grade-css';
    st.textContent =
      '.clip_canvas.is-graded .gameplay_hold,' +
      '.clip_canvas.is-graded .facecam_hold,' +
      '.clip_canvas.is-graded .chat_hold{filter:' + _gradeFilterRaw(true) + ';}';
    document.head.appendChild(st);
  }
  function _applyGradeClass() {
    document.querySelectorAll('.clip_canvas').forEach(cc => cc.classList.toggle('is-graded', !!_gradeEnabled));
  }
  // Grade on/off button ([data-grade] / [wized="grade_button"] / #grade_button) — toggles + reflects
  // the video-layer grade. Polled so it catches a Webflow button that mounts late; a shared listener
  // keeps every instance visually in sync (class .is-active + aria-pressed).
  function _syncGradeButtons() {
    const on = !!window.canvasAPI?.getGradeEnabled?.();
    document.querySelectorAll('[data-grade], [wized="grade_button"], #grade_button').forEach(btn => {
      btn.classList.toggle('is-active', on);
      btn.setAttribute('aria-pressed', on ? 'true' : 'false');
      if (!btn._gradeBound && window.canvasAPI) {
        btn._gradeBound = true;
        btn.addEventListener('click', (e) => { e.preventDefault(); e.stopPropagation(); window.canvasAPI.toggleGrade?.(); });
      }
    });
  }
  window.addEventListener('gradeChanged', _syncGradeButtons);
  setInterval(_syncGradeButtons, 800);

  // Per-clip SPEED buttons — visually mirror the layout/mode group (a #speed_select_group of
  // .editor_config-option buttons with wized="speed-050/075/100/150/200"), but wired self-contained
  // like the grade button (click → setEditClipSpeed, no Wized workflow needed). Also accepts a plain
  // [data-speed="0.5"] hook. is-active reflects the PLAYHEAD clip's speed and re-syncs as the playhead
  // crosses clips (clipChanged) or the model changes (clipsChanged); disabled when there's no clip.
  const SPEED_MAP = { 'speed-050': 0.5, 'speed-075': 0.75, 'speed-100': 1, 'speed-150': 1.5, 'speed-200': 2 };
  function _speedBtnVal(btn) {
    const w = btn.getAttribute('wized');
    if (w && SPEED_MAP[w] != null) return SPEED_MAP[w];
    if (btn.dataset.speed != null) { const v = +btn.dataset.speed; return isFinite(v) && v > 0 ? v : null; }
    return null;
  }
  function _syncSpeedButtons() {
    const api = window.canvasAPI; if (!api) return;
    const id  = api.getEditClip?.();
    const cur = id ? (api.getClipSpeed?.(id) ?? 1) : 1;
    document.querySelectorAll('#speed_select_group .editor_config-option, [data-speed]').forEach(btn => {
      const val = _speedBtnVal(btn);
      if (val == null) return;
      btn.classList.toggle('is-active', Math.abs(val - cur) < 1e-3);
      btn.classList.toggle('is-disabled', !id);
      btn.setAttribute('aria-pressed', Math.abs(val - cur) < 1e-3 ? 'true' : 'false');
      if (!btn._speedBound && window.canvasAPI) {
        btn._speedBound = true;
        btn.addEventListener('click', (e) => {
          e.preventDefault(); e.stopPropagation();
          if (!window.canvasAPI?.getEditClip?.()) { console.warn('[speed] seek the playhead into a clip first'); return; }
          window.canvasAPI.setEditClipSpeed?.(val);
          _syncSpeedButtons();
        });
      }
    });
  }
  window.addEventListener('clipChanged',  _syncSpeedButtons);   // playhead crossed into another clip
  window.addEventListener('clipsChanged', _syncSpeedButtons);   // split/delete/speed reflow
  setInterval(_syncSpeedButtons, 800);                          // catch late-mounting Webflow buttons
  ...
  function bindTrack(video, el, getWin, useOutput) {
    if (!el) return;
    // Image/subtitle windows are stored in SOURCE time — the timeline ruler is source space and the default
    // window IS the source window (e.g. a VOD clip's 430–471s), so they gate by SOURCE time. The TITLE
    // (useOutput=true) is an OUTPUT-time intent ("first N seconds of the FINAL cut"): gating it by output
    // time makes "start at 0s" follow the clip through reorders instead of freezing to a source in-point.
    // (Output gating only "broke VOD" historically when a SOURCE-valued window was compared to the output
    // clock — a unit mismatch; the title window is stored in output time now, so the units line up.)
    const at = (sec) => {
      let t; try { t = getWin(); } catch (_) { return; }   // getter may reference state still in TDZ at bind time
      if (!t) return;
      const on = sec >= (t.start ?? 0) && sec <= (t.end ?? Infinity);
      el.style.opacity       = on ? '1' : '0';
      el.style.pointerEvents = on ? ''  : 'none';
    };
    // compositor callback carries (srcSec, outputSec); engine-off fallback uses source (≈ output on a
    // 0-based segment clip, which is when the engine isn't driving).
    (window.wcTimeLayers ||= []).push((srcSec, ot) => at(useOutput && ot != null ? ot : srcSec));
  ...
  function _bindFx(video, frameEl) {
    if (!frameEl) return;
    _ensureGradeCSS(); _applyGradeClass();      // video-layer grade (CSS rule + .is-graded class)
    ...
    window.canvasAPI = {
      ...
      getModeSegments() {
        ...
        for (const c of ordered) {
          const span = (c.sourceEnd - c.sourceStart) / this.clipSpeed(c);
          if (!(span > 0)) continue;
      ...
      // Title visibility WINDOW in OUTPUT time (the final cut's timeline). Gated by output time in BOTH the
      // preview (bindTrack useOutput=true) and the render, so "start at 0s" follows the clip to output-0
      // through any reorder — it's no longer frozen to a source in-point. end=null → show to the end.
      setTitleTiming(startOut, endOut) {
        if (!tracks.title) tracks.title = { start: 0, end: null };
        tracks.title.start = Math.max(0, +startOut || 0);
        tracks.title.end   = (endOut == null) ? null : Math.max(tracks.title.start + 0.1, +endOut);
      ...
      getEditClip() {
        // Edit target ALWAYS follows the playhead — seek to a clip (or click it, which seeks) and edits
        // land on it. No pin-to-edit: clicking to select is cosmetic / for delete only. Base (null) only
        // when there are no clips. (Was: a pinned .is-selected clip overrode the playhead.)
        return this.getPlayheadClipId();
      },
      ...
      // The EFFECTIVE track at the playhead clip (its own override.tracking, else base reframe) — so the
      // tracking HUD / marker reflect AI & per-clip tracks, not only the base reframe. raw = unsmoothed pts.
      getEffectiveTracking() {
        const id = this.getPlayheadClipId();
        if (id) {
          const t = (tracks.video.clips || []).find(c => c.id === id)?.override?.tracking;
          if (t && t.enabled && t.raw && t.raw.length) return { enabled: true, raw: t.raw, mode: t.mode || 'face', src: 'clip' };
          if (t && t.enabled === false) return { enabled: false, raw: [], mode: null, src: 'clip' };   // explicitly off on this clip
        }
        const rf = window.wcReframe;
        if (rf?.enabled && rf.raw?.length) return { enabled: true, raw: rf.raw, mode: rf.mode || 'face', src: 'base' };
        return { enabled: false, raw: [], mode: null, src: null };
      },
      // Smoothed look position (cx,cy) of the effective track at source time t — the SAME baked path the
      // frame pan uses (_bakedTrackFor), so the on-canvas marker sits exactly where the crop is looking.
      sampleEffectiveTracking(t) {
        try {
          const id = this.getPlayheadClipId();
          const clip = id ? (tracks.video.clips || []).find(c => c.id === id) : null;
          const baked = _bakedTrackFor(clip);
          return (baked && baked.length) ? _interpTrack(baked, t) : null;
        } catch (_) { return null; }
      },
      ...
      // Bulk-tag clips with the character each is tracking (map: clipId → charId). One setClips → one
      // clipsChanged. Purely a UI/persistence tag — the frame still reads override.tracking, so it can't
      // change the edit. Used by the AI director's tracked-face relay to light the right library slot.
      setClipsCharacters(map) {
        if (!map || !Object.keys(map).length) return;
        const clips = (tracks.video.clips || []).map(c =>
          map[c.id] ? { ...c, override: { ...(c.override || {}), character: map[c.id] } } : c);
        this.setClips(clips, false);
        window.dispatchEvent(new CustomEvent('charactersChanged'));
      },
      // Per-clip playback SPEED multiplier. A clip at speed S plays its source (sourceEnd−sourceStart)
      // in srcDur/S of OUTPUT time (2× → half as long on the timeline; 0.5× → twice as long). Stored on
      // override.speed so it rides the same per-clip override bag as layout/mode/tracking; default 1,
      // clamped [0.25, 4]. This one accessor is the single source of truth every output-time seam reads.
      clipSpeed(c) {
        const s = +(c?.override?.speed);
        return (isFinite(s) && s > 0) ? Math.min(4, Math.max(0.25, s)) : 1;
      },
      getOutputDuration() {
        const clips = tracks.video.clips || [];
        if (!clips.length) return 0;
        return clips.reduce((acc, c) => Math.max(acc, c.outputStart + (c.sourceEnd - c.sourceStart) / this.clipSpeed(c)), 0);
      },
      ...
      getOutputClips() {
        return (tracks.video.clips || [])
          ...
          .map(c => ({ ...c, outputEnd: c.outputStart + (c.sourceEnd - c.sourceStart) / this.clipSpeed(c) }));
      },
      // Set a clip's playback speed and reflow the contiguous output timeline. Because this clip's output
      // duration (srcDur/speed) changes, every downstream clip's outputStart shifts — same ripple model as
      // delete/move. Fires clipsChanged (→ timeline re-lays bars, engine re-times next tick) and nudges the
      // live engine to reschedule audio + re-seek video at the current instant under the new mapping.
      setClipSpeed(clipId, speed) {
        const clips = (tracks.video.clips || []).slice();
        const i = clips.findIndex(c => c.id === clipId);
        if (i < 0) return false;
        const s = (isFinite(+speed) && +speed > 0) ? Math.min(4, Math.max(0.25, +speed)) : 1;
        clips[i] = { ...clips[i], override: { ...(clips[i].override || {}), speed: s } };
        const ordered = clips.slice().sort((a, b) => (a.outputStart || 0) - (b.outputStart || 0));
        let cur = 0;
        const reflowed = ordered.map(c => {
          const nc = { ...c, outputStart: cur };
          cur += (c.sourceEnd - c.sourceStart) / this.clipSpeed(c);
          return nc;
        });
        this.setClips(reflowed);
        // Repaint the clip bars from the model (widths/positions changed but the track width didn't, so the
        // ResizeObserver won't fire). Lighter than a full remount and won't clobber an in-progress drag.
        try { window._timelineRelayoutBars?.(); } catch (_) {}
        try { const e = window.wcEngine; if (e?.isActive?.()) e.seekOutput(e.currentOutputTime()); } catch (_) {}
        return true;
      },
      getClipSpeed(clipId) {
        const c = (tracks.video.clips || []).find(x => x.id === clipId);
        return c ? this.clipSpeed(c) : 1;
      },
      // Convenience for console/UI: retarget the clip under the playhead.
      setEditClipSpeed(speed) {
        const id = this.getEditClip();
        if (!id) { console.warn('[speed] no clip under playhead — seek into a clip first'); return false; }
        return this.setClipSpeed(id, speed);
      },
      ...
      addSoundboardSound(url, at, meta = {}) {
        ...
        const entry = { id, url, at: _anchorStore(at, SOUNDBOARD_ANCHOR), name: meta.name || '', overlay: meta.overlay || null, buffer: null, duration: 0 };
      ...
      moveSoundboardSound(id, at) {
        ...
        if (!s) return;
        s.at = _anchorStore(at, SOUNDBOARD_ANCHOR);   // timeline (output) drop → footage (source) anchor
        window.dispatchEvent(new CustomEvent('soundboardChanged'));
      },
      ...
      addImage(url, at, meta = {}) {
        ...
        _images.push({
          id, url,
          at:    _anchorStore(at, IMAGE_ANCHOR),
      ...
      moveImage(id, at) {
        ...
        if (!im) return;
        im.at = _anchorStore(at, IMAGE_ANCHOR);   // timeline (output) drop → footage (source) anchor
        window.dispatchEvent(new CustomEvent('imagesChanged'));
      },
      ...
      getImageCues() {
        ...
        const resolveOut = (at) => IMAGE_ANCHOR === 'source'
          ? (window.canvasAPI?.sourceTimeToOutputTime?.(at) ?? null)   // pinned to footage → source→output (null if cut away)
          : Math.max(0, at);                                            // 'output' → stored value IS the timeline position
        const live = _images
          .map(im => ({ im, outputStart: resolveOut(im.at) }))
          .filter(x => x.outputStart != null && isFinite(x.outputStart) && x.outputStart < cap)
          .sort((a, b) => a.outputStart - b.outputStart);
        const out = [];
        for (let i = 0; i < live.length; i++) {
          const x = live[i], next = live[i + 1];
      ...
      // Clip grade — the video-layer look filter. Render reads getGradeFilter (same numbers as
      // the live CSS rule) so export == preview. withSharpen=false = colour-only (render appends
      // the url() sharpen itself, gated on canvas url()-filter support). Returns '' when toggled off.
      getGradeFilter(withSharpen = true) { return _gradeFilter(withSharpen); },
      getClipGrade() { return { ...CLIP_GRADE }; },
      getGradeEnabled() { return _gradeEnabled; },
      setGradeEnabled(on) {
        _gradeEnabled = !!on;
        _ensureGradeCSS(); _applyGradeClass();           // live: flip the .is-graded class
        window.dispatchEvent(new CustomEvent('gradeChanged', { detail: { enabled: _gradeEnabled } }));
        window._canvasPersistTrySave?.();                // persist the per-clip choice
        return _gradeEnabled;
      },
      toggleGrade() { return this.setGradeEnabled(!_gradeEnabled); },
  ...
  function initCanvas(item, opts = {}) {
    ...
    bindTrack(gpVideo, titleHold, () => tracks.title, true);   // TITLE window is OUTPUT-time (follows reorders)
