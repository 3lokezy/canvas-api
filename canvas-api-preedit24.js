  ...
  function initCanvas(item, opts = {}) {
    ...
    (function bindImageCues() {
      ...
      const apply = (ot) => {
        ...
        if (!cues.length) {                                         // no cues left → relinquish to legacy path,
          if (_shownId !== null) {                                 // but first HIDE if we were showing a cue
            imgHold.style.opacity = '0'; imgHold.style.visibility = 'hidden';
            imgHold._userVisible = false; _shownId = null; imgHold._activeCueId = null;
          }
          return;
        }
        if (!cue) {
        ...
        // Set tracking + visibility FIRST — these can't throw, so the hide/swap path stays correct even if
        // geometry throws (wcTimeLayers wraps each layer in a silent try/catch, so an earlier throw here left
        // _shownId null → the `!cue` hide branch never fired → the image never cleared).
        _shownId = cue.id;
