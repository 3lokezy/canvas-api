(() => {
  ...
  function createEngine(gpVideo, cv, media) {
    ...
    let _lastDrawnFrameTs = 0, _nullDraws = 0, _syncThrottle = 0;   // [wc sync] debug only
    ...
    function installBgMirror(bgEl) {
      ...
      m.className = bgEl.className;                 // inherit .bg_video CSS (box, border-radius, blur)
      ...
      _bgMir = { el: bgEl, cv: m, ctx: m.getContext('2d'), prevDisplay };
    }
    function drawBgMirror() {
      ...
      if (!cw || !ch) return;
      const scale = Math.max(pw / cw, ph / ch);     // object-fit: cover (center-crop), as before
      const dw = cw * scale, dh = ch * scale;
      mx.drawImage(cv, (pw - dw) / 2, (ph - dh) / 2, dw, dh);
    }
    ...
    function tick() {
      ...
      if (!clip) {
        ...
        // Video-only A/V trim: shift just the video clock by wcVideoOffsetMs (audio + subtitles
        // stay on the true clock). Tune live by eye, then bake the value. + = video shows later
        // frames sooner (pulls video forward, e.g. to compensate audio output latency).
        const _vOff = (window.wcVideoOffsetMs || 0) * 1000;
        if (main) {
          main.extendTo(srcUs + _vOff);
          const f = main.present(srcUs + _vOff);
          if (f) { _lastDrawnFrameTs = f.timestamp; draw(f); f.close(); }
          else _nullDraws++;
          if (window.wcSyncDebug && (_syncThrottle = (_syncThrottle + 1) % 30) === 0) {
            const targetSrcSec = (srcUs + _vOff) / 1e6;
            const drawnSrcSec  = _lastDrawnFrameTs / 1e6;
            console.log('[wc sync]',
              'ot=' + ot.toFixed(2),
              'targetSrc=' + targetSrcSec.toFixed(3),
              'drawnSrc=' + drawnSrcSec.toFixed(3),
              'vGap(=behind clock)=' + ((targetSrcSec - drawnSrcSec) * 1000).toFixed(0) + 'ms',
              'presentQ=' + main.presentQ.length,
              'decQ=' + (main.decoder ? main.decoder.decodeQueueSize : -1),
              'nullDraws=' + _nullDraws,
              'actxLatency=' + (((actx.baseLatency || 0) + (actx.outputLatency || 0)) * 1000).toFixed(0) + 'ms');
          }
        }
