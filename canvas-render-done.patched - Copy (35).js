      ...
      rv.addEventListener('seeked', async function onSeeked() {
        ...
        async function pumpEmits(currentMediaTime) {
          ...
          while (nextEmitIndex < totalFrames && (nextEmitIndex / FPS) <= offset + 1e-6) {
            // Sync rv to this frame's timestamp — rv may be externally paused/frozen.
            // Setting currentTime on a paused element updates the displayed frame fast
            // enough for the subsequent drawCanvasFrame call.
            const frameT = effectiveStart + nextEmitIndex / FPS;
            if (Math.abs(rv.currentTime - frameT) > 0.002) rv.currentTime = frameT;
