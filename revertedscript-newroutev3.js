(function () {
    ...
    function applyMode(mode, refs) {
      ...
      // Default zoom applied by applyMode — will be overridden by per-mode restore if available
      requestAnimationFrame(() => {
    ...
    function buildPanelAPI(refs) {
      const {
        ...
        modeStates, saveModeState, restoreModeState,
      } = refs;
      ...
      window.canvasAPI = {
        ...
        setModeStates(states) {
          Object.keys(states).forEach(mode => {
            if (modeStates[mode]) Object.assign(modeStates[mode], states[mode]);
          });
        },
        ...
        getState() {
          ...
          // Keep current mode state fresh in modeStates before saving
          saveModeState(frame._currentMode);
    ...
    function initCanvas(item, opts = {}) {
      ...
      // Saves zoom + pan per mode so switching back restores where you left off
      // is-split zoom is always locked to 1 so nothing to save there
      const modeStates = {
      ...
      function saveModeState(mode) {
        if (mode !== 'is-full' && mode !== 'is-overlay') return;
        const gpLeft = parseFloat(gpVideo.style.left) || 0;
        const gpTop  = parseFloat(gpVideo.style.top)  || 0;
        const gpHW   = gpHold.clientWidth  || 1;
        const gpHH   = gpHold.clientHeight || 1;
        modeStates[mode] = {
          zoom: gpZoom,
          panX: gpHW ? gpLeft / gpHW : 0,
          panY: gpHH ? gpTop  / gpHH : 0,
        };
      }
    
      function restoreModeState(mode) {
        if (mode !== 'is-full' && mode !== 'is-overlay') return false;
        ...
        gpVideo.style.left = `${Math.max(gpHW - gpW, Math.min(0, saved.panX * gpHW))}px`;
        gpVideo.style.top  = `${Math.max(gpHH - gpH, Math.min(0, saved.panY * gpHH))}px`;
        return true;
      }
      ...
      if (gpHold && gpVideo) {
        ...
        gpVideo.addEventListener('loadedmetadata', () => {
            createBgVideo();
            // Clear cover placeholder, let applyGpZoom take full control
            gpVideo.style.removeProperty('object-fit');
            // Always apply zoom now — real dimensions available
            // applyModeFn's double-RAF will override with saved modeState if present
            applyGpZoom(gpZoom);
            const { effectiveStart } = getTrackBounds();
        ...
        // Prevent distortion before metadata loads — JS takes over on loadedmetadata
      ...
      function bindFcDrag() {
        ...
        function onUp() { if (!dragLive) return; dragLive = false; fcHold.style.cursor = 'grab'; }
      ...
      // ── Mode API — saves current mode state before switching, restores after ──
      const modeRefs = {
      ...
      function applyModeFn(mode) {
        ...
        // After applyMode's internal default-zoom RAF fires, restore saved mode state
        requestAnimationFrame(() => {
          requestAnimationFrame(() => {
            restoreModeState(frame._currentMode);
          });
        });
      }
