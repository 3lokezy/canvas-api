(function () {
  ...
  // ── Caption emphasis: the SALIENT word per phrase (deterministic, no LLM, no keywords needed) ─────
  // Every clip gets ~one tasteful "money word" pop per sentence — the longest CONTENT word (stopwords
  // skipped), with shouted/emphatic words scoring higher. Returns a Set of word START times (ms, unique
  // per word); memoised on the transcript. Defined at LOAD here (core module) so BOTH surfaces agree:
  // the live-preview DOM below + the canvas export in canvas-render. Tunable live via window._capEmphCfg.
  window._capEmphCfg = window._capEmphCfg || { color: '#ffe14d', scale: 1.14 };
  window._capEmphWords = window._capEmphWords || (function () {
    const STOP = new Set(['this','that','with','your','from','they','them','were','have','what','when','then','than',
      'into','just','like','about','really','gonna','their','there','which','would','could','because','been','some',
      'you','and','the','but','for','are','was','not','all','how','did','get','got','out','one','his','her','him',
      'she','who','why','can','yeah','okay','know','went','goes','said','says','over','onto','they','are','our']);
    const norm = s => String(s || '').toLowerCase().replace(/[^a-z0-9]/g, '');
    let _t = null, _n = -1, _cached = new Set();
    return function (tr) {
      if (!Array.isArray(tr) || !tr.length) return new Set();
      if (tr === _t && tr.length === _n) return _cached;
      const set = new Set();
      let phrase = [];
      const flush = () => {
        ...
        for (const w of phrase) {
          ...
          let score = n.length;
          const raw = String(w.text).replace(/[^A-Za-z0-9]/g, '');
          if (raw.length >= 3 && raw === raw.toUpperCase() && /[A-Z]{2}/.test(raw)) score += 6;   // SHOUTED
          if (/[!?]$/.test(w.text)) score += 2;                                                    // emphatic
          if (score > bestScore) { bestScore = score; best = w; }
        }
        if (best) set.add(best.start);
        phrase = [];
      };
      for (let i = 0; i < tr.length; i++) {
        phrase.push(tr[i]);
        const endsSentence = /[.!?]$/.test(tr[i].text || '');
        const gap = i < tr.length - 1 ? (tr[i + 1].start - tr[i].end) : Infinity;   // start/end are ms
        if (endsSentence || gap > 600 || phrase.length >= 7) flush();
      }
      flush();
      _t = tr; _n = tr.length; _cached = set;
      return set;
    };
  })();
  ...
  function bindSubtitles(video, transcript, pillEl, textEl) {
    ...
    const emphSet = window._capEmphWords ? window._capEmphWords(transcript) : new Set();   // salient-word starts
    let lastIdx = -2;
    ...
    function sample(ms) {
      ...
      if (idx !== lastIdx) {
        ...
        if (idx !== -1) {
          ...
          const emph = emphSet.has(transcript[idx].start);                                  // salient word → colour
      ...
      if (idx !== -1 && pillEl) {
        ...
        const eScale = emphSet.has(transcript[idx].start) ? (window._capEmphCfg?.scale || 1.14) : 1;
    ...
    function renderChunk(chunkIdx, activeWordIdx) {
      ...
      if (chunkIdx !== lastChunkIdx) {
        ...
        chunk.forEach((word, i) => {
          ...
          const emph = emphSet.has(word.start);                                 // colour-only in chunk mode (matches export)
