  ...
  function initCanvas(item, opts = {}) {
    ...
    (function bindImageCues() {
      ...
      const apply = (ot) => {
        ...
        // (cold load, raw gpVideo playback that fires timeupdate), HIDE rather than placing them by source time,
        // which would show/loop a GIF prematurely while the timeline is still loading. isActive stays true when
        // the engine is merely paused, so paused-with-cues still renders correctly.
        if (!window.wcEngine?.isActive?.()) { if (_shownId !== null) hide(false); return; }
