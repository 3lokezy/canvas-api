(function () {
  ...
  function makeOverlayInteractive(hold, frame, getScale) {
    ...
    if (resizable && !hold.querySelector('[data-overlay-handle]')) {
      ...
      if (_COARSE) Object.assign(dot.style, { opacity: '1', width: '18px', height: '18px', bottom: '-9px', right: '-9px' });
      hold.appendChild(dot);

      hold.addEventListener('pointerenter', () => { dot.style.opacity = '1'; });
      hold.addEventListener('pointerleave', () => { if (!resizeLive && !_COARSE) dot.style.opacity = '0'; });
  ...
  function initCanvas(item, opts = {}) {
    ...
    function bindFcDrag() {
      ...
      fcHold.addEventListener('pointerleave', () => { if (!resizeLive && !_COARSE) dot.style.opacity = '0'; });
