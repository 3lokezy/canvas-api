(() => {
  ...
  const BG_SMOOTH       = 0.12;   // bg mirror temporal blend (per-frame alpha); lower = smoother/slower
  ...
  function createEngine(gpVideo, cv, media) {
    ...
    function installBgMirror(bgEl) {
      ...
      _bgMir = { el: bgEl, cv: m, ctx: m.getContext('2d'), prevDisplay, primed: false };
    }
    function drawBgMirror() {
      ...
      if (m.width !== pw || m.height !== ph) { m.width = pw; m.height = ph; _bgMir.primed = false; }
      ...
      // Temporal smoothing: blend each frame in at low alpha so the blurred bg eases between frames
      // instead of flashing dark↔bright as the camera moves. First frame (and after a resize that
      // clears the canvas) draws solid so it doesn't fade up from black. window.wcBgSmooth overrides
      // BG_SMOOTH live (console-tunable: set 0.04–0.3 while playing, then bake the value you like).
      const sm = (typeof window.wcBgSmooth === 'number') ? window.wcBgSmooth : BG_SMOOTH;
      mx.globalAlpha = _bgMir.primed ? sm : 1;
