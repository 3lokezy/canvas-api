(function () {
  ...
  async function _supabaseWriteRenderId(clipId, renderId) {
    try {
      ...
      if (!token) return;
      const _clipType = new URLSearchParams(window.location.search).get('type') || 'live';
      const _table    = _clipType === 'vod' ? 'clips' : 'clips_live';
      await fetch(
  ...
  if (bR) {
    ...
    if (useMp4) {
      ...
      try {
        ...
        if (isInSource && typeof gpVideo.captureStream === 'function') {
