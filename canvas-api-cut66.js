(function () {
  ...
  function buildPanelAPI(refs) {
    ...
    let _characters    = [];     // user-built identity library: [{ id, emb:[…], thumb, name }]
    ...
    window.canvasAPI = {
      ...
      setCharacterTrack(charId, raw) {
        const editClip = this.getEditClip();
        if (!editClip || !raw || !raw.length) { if (window.wcReframe) { window.wcReframe.setTrack(raw || []); window.wcReframe.enable(!!(raw && raw.length)); } return; }
        const clips = (tracks.video.clips || []).map(c => {
      ...
      setClipTrackingOff() {
        const editClip = this.getEditClip();
        if (!editClip) { if (window.wcReframe) window.wcReframe.enable(false); window.dispatchEvent(new CustomEvent('charactersChanged')); return; }
        const clips = (tracks.video.clips || []).map(c => c.id === editClip
      ...
      getClipTrackingInfo() {
        const id = this.getEditClip(); if (!id) return null;
