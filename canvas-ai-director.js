        ...
        if (s.mode === 'is-split') {
          try {
            ...
            if (box) {
              ...
              // No composable second subject AND no real cam (n8n row / Supabase DOM / scan corner) to ride:
              // a split here would show one person twice or an empty region. Downgrade to full — featured-person
              // punch when validate framed one, honest wide otherwise.
              c.override.mode = 'is-full';
              console.log('[ai-polish] split→full ' + s.from.toFixed(1) + 's — no distinct 2nd subject on screen');
            }
