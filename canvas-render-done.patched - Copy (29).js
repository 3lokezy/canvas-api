(function () {
  ...
  const _RENDER_SCRIPT_BUILD = 'pb-20260505n-diag';
    ...
    return new Promise((resolve, reject) => {
      ...
      rv.addEventListener('seeked', async function onSeeked() {
        ...
        if (_iosSeekMode) {
          ...
          // For full-length clips, the video's 'ended' event fires and rVFC stops
          // delivering callbacks — the post-pumpEmits rv.ended check inside onVideoFrame
          // is never reached. Use a polling guard rather than an 'ended' event listener
          // to avoid stale events left in the task queue from a previous render firing
          // our listener immediately and triggering finalizeRender before any frames run.
          ...
          _rvfcGuardInterval = setInterval(async () => {
            ...
            // Only trigger if rVFC has been silent for 500 ms AND video has ended.
            if (rv.ended && (Date.now() - _lastRvfcFiredAt) > 500) {
          ...
          console.warn('Render: requestVideoFrameCallback unavailable — using setInterval fallback (CFR enforced via frame index, but pacing depends on timer accuracy)');
          const onTimerTick = () => {
