...
if (kickLogo && barEl) {
  ...
  if (bR) {
    ...
    // Pan rect recomputed PER FRAME from gpVideo's live left/top. The auto-reframe
    // sampler (wcTimeLayers) animates those during the engine-capture playthrough,
    // so reading them each frame keeps the rendered crop in lockstep with preview.
    // (editorHW/HH/DW/DH are fixed during a render; only left/top move.)
    function _gpPanRect() {
