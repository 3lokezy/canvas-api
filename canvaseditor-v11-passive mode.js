...
function buildPanelAPI(refs) {
  ...
  window.canvasAPI = {
    ...
    setMasterTrim(trimIn, trimOut) {
        ...
        // Duration not ready — store raw, apply after loadedmetadata
        if (!duration) {
      ...
      // Sticky secondary tracks — move with master if within snap threshold
      Object.keys(tracks).forEach(name => {
        if (name === 'video') return;
        const t          = tracks[name];
        const trackStart = t.start ?? oldStart;
        const trackEnd   = t.end   ?? oldEnd;
        if (Math.abs(trackStart - oldStart) <= SNAP) {
          t.start = trackStart + deltaStart;
        }
        if (t.end !== null && Math.abs(trackEnd - oldEnd) <= SNAP) {
          t.end = trackEnd + deltaEnd;
        }
      });
    ...
    setTrack(name, { start, end } = {}) {
        ...
        const duration = gpVideo?.duration || 0;
        if (!duration) {
          // Duration not ready — store raw values, clamping deferred
...
// ── Passive mode — apply basic layout so video is visible ───────────────
if (!interactive) {
  // Apply mode classes based on crop data already read by reapplyCrop
  const passiveMode = (item._cropValid) ? 'is-split' : 'is-full';
  frame.classList.add(passiveMode);
  gpHold?.classList.add(passiveMode);
  fcHold?.classList.add(passiveMode);

  // Size video to fill its hold
  if (gpHold && gpVideo) {
    ...
    gpVideo.addEventListener('loadedmetadata', sizeVideo, { once: true });
    if (gpVideo.readyState >= 1) sizeVideo();
  }
  return;
}
...
// Guard: skip if duration not yet known to avoid collapsing elements on load
...
function reapplyCrop(item) {
  ...
  const frame = item.querySelector('.clip_canvas');
  const isPassive = !item._applyMode;
  ...
  if (cropValid) {
    ...
    if (!wasValid) {
      item._applyMode?.('is-split');
      if (isPassive) {
        ['is-full', 'is-overlay'].forEach(m => { frame?.classList.remove(m); gpHold?.classList.remove(m); fcHold?.classList.remove(m); });
    ...
    if (wasValid !== false) {
      ...
      if (isPassive) {
        const passiveMode = item._hasChatSplit ? 'is-split' : 'is-full';
        ['is-full', 'is-split', 'is-overlay'].forEach(m => { frame?.classList.remove(m); gpHold?.classList.remove(m); fcHold?.classList.remove(m); });
        frame?.classList.add(passiveMode);
        gpHold?.classList.add(passiveMode);
        fcHold?.classList.add(passiveMode);
        if (fcHold) fcHold.style.display = (passiveMode !== 'is-full') ? '' : 'none';
      }
    }
  }
