(function () {
  ...
  function _bindFx(video, frameEl) {
    ...
    const apply = (ot) => {
      ...
      flash.style.background = fx.flashColor || 'transparent';
      flash.style.opacity    = fx.flashAlpha > 0 ? String(fx.flashAlpha) : '0';
      frameEl.style.filter   = fx.desat > 0 ? `saturate(${(1 - fx.desat).toFixed(3)})` : '';
    };
    ...
    window.addEventListener('fxChanged', () =>
  ...
  function buildPanelAPI(refs) {
    ...
    window.canvasAPI = {
      ...
      addSoundboardSound(url, at, meta = {}) {
        ...
        const entry = { id, url, at: Math.max(0, +at || 0), name: meta.name || '', buffer: null, duration: 0 };
      ...
      getSoundboardSounds() { return _soundboard.map(s => ({ id: s.id, url: s.url, at: s.at, name: s.name, duration: s.duration })); },
      async setSoundboardSounds(arr) {
        _soundboard = (arr || []).map(s => ({ id: s.id || ('sb_' + Math.random().toString(36).slice(2, 8)), url: s.url, at: Math.max(0, +s.at || 0), name: s.name || '', buffer: null, duration: 0 }));
        window.dispatchEvent(new CustomEvent('soundboardChanged'));
      ...
      getSoundboardCues() {
        ...
        for (let i = 0; i < live.length; i++) {
          ...
          if (effDur > SOUNDBOARD_MIN_GAP) {
            out.push({ id: x.s.id, url: x.s.url, name: x.s.name, outputStart: x.out, effDur, duration: intrinsic, buffer: x.s.buffer });
          }
        }
        return out;
      },
      ...
      fxIntensityAt(ot) {
        ...
        for (let i = 0; i < _fx.length; i++) {
          const c = _fx[i];
          const e = ot - c.at;
          if (e <= 0 || e >= c.dur) continue;
          const env = _fxEnv(c.kind, e, c.dur);
          if (c.kind === 'desat') { if (env > desat) desat = env; }
          else { const a = env * c.peak; if (a > flashAlpha) { flashAlpha = a; flashColor = c.color; } }
        }
      ...
      getState() {
        ...
        return {
          ...
          soundboard:  _soundboard.map(s => ({ id: s.id, url: s.url, at: s.at, name: s.name })),
