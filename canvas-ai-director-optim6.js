(function () {
  ...
  const VERSION = 'v2.6-beats-letterbox';
  ...
  function generatePlan() {
    ...
    for (const t of merged) {
      ...
      // reaction cutaway: the listener's most animated non-first beat
      let reactIdx = -1;
      if (other && bounds.length > 2) {
        let best = 0.09;
        for (let i = 1; i < bounds.length - 1; i++) {
          const m = mmIn(other, bounds[i], bounds[i + 1]);
      ...
      for (let i = 0; i < bounds.length - 1; i++) {
        ...
        let s = null;
        if (i === reactIdx && other) {
          s = { from: a, to: b, mode: 'is-full', person: other, zoom: +Math.min(CFG.ZOOM_MAX, CFG.TRACK_PUNCH + 0.2).toFixed(2), react: true };
        } else if (ctx === 'duo') {
