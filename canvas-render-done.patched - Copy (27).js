(function () {
  ...
  const _RENDER_SCRIPT_BUILD = 'pb-20260502l-rvfc-ended';
    ...
    return new Promise((resolve, reject) => {
      ...
      rv.addEventListener('seeked', async function onSeeked() {
        ...
        if (_iosSeekMode) {
          ...
          // is never reached. Track the most recent pumpEmits promise so the 'ended'
          // handler can wait for it to settle before calling finalizeRender.
          let _lastPumpPromise = Promise.resolve();

          rv.addEventListener('ended', async () => {
