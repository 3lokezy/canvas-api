(function () {
  ...
  function bindTrack(video, el, getWin) {
    if (!el) return;
    // Title/image/subtitle visibility windows are set in TIMELINE (output) space — "show 0–5s of the
    // final video". So gate by OUTPUT time, not source: otherwise after a cut/reorder a window of
    // output 0–5 matches whatever SOURCE range plays then (e.g. a clip moved to 30s shows it at
    // 30–35s). Under the engine the layer gets (srcSec, outputSec); the legacy non-engine fallback
    // has no NLE remap, so source time == timeline time there (video.currentTime).
    const at = (sec) => {
    ...
    (window.wcTimeLayers ||= []).push((srcSec, outputSec) => at(outputSec != null ? outputSec : srcSec));
