(function () {
  ...
  function mountNLETrack(masterRow, api, getDisplayDuration) {
    ...
    // Clip thumbnail: show this clip's [sourceStart,sourceEnd] slice of the master
    // filmstrip (window.wcThumbs) via CSS background — resize = recompute 2 props.
    function applyThumb(b) {
      const t = window.wcThumbs;
      const div = b.element.querySelector('.clip_thumbs');
      if (!div) return;
      if (!(t && t.ready && t.url && t.span > 0)) return;
      const bw = b.width || b.element.clientWidth;
      const s0 = b.sourceIn * masterDuration, s1 = b.sourceOut * masterDuration;
      const fw = (s1 - s0) / t.span;
      if (!bw || fw <= 0) return;
      const sizeW = bw / fw;
      div.style.backgroundImage    = `url("${t.url}")`;
      div.style.backgroundRepeat   = 'no-repeat';
      div.style.backgroundSize     = `${sizeW}px 100%`;
      div.style.backgroundPosition = `${-((s0 - t.base) / t.span) * sizeW}px center`;
    }
    ...
    function endAction() {
      ...
      if (moved) {
        // Glide the dropped clip into its final slot too, then clear transitions.
