(function () {
  ...
  function buildPanelAPI(refs) {
    ...
    window.canvasAPI = {
      ...
      setFacecamCrop(box) {
        if (!box) { item._cropInjected = null; reapplyCrop(item); return; }
        const { x1, y1, x2, y2 } = box;
        if (![x1, y1, x2, y2].every(Number.isFinite) || !(x2 > x1) || !(y2 > y1)) return;
        item._cropInjected = { x1, y1, x2, y2 };
        reapplyCrop(item);
      },
      ...
      getState() {
        ...
        return {
          ...
          facecamCrop: item._cropInjected || null,   // scan-injected crop (only set when Supabase had none)
  ...
  function reapplyCrop(item) {
    ...
    if (!box && item._cropInjected) box = item._cropInjected;
    const cropValid = !!box;
