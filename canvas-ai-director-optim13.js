    ...
    for (const t of work) {
      ...
      for (let i = 0; i < bounds.length - 1; i++) {
        ...
        if (feature === 'wide') {
          ...
          // on a cam clip a personless punch lands on the CAM person — the streamer blown up full-screen
          s = { from: a, to: b, mode: 'is-full', person: t.fperson || feat || camP, zoom: +Math.min(CFG.ZOOM_MAX, esc).toFixed(2) };
