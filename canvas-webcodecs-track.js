(() => {
    ...
    function _patchTrack(perFrame, gw, gh, { sx, sy, atSec, patchFrac = TRACK.patchFrac, searchFrac = TRACK.searchFrac, step = TRACK.step, adapt = TRACK.adapt, lostThresh = TRACK.lostThresh, recenter = TRACK.recenter }) {
      ...
      const walk = (from, to, dir) => {
        let pcx = a.cx, pcy = a.cy, tref = (dir < 0) ? a.t.slice() : a.t;
        for (let i = from; dir > 0 ? i < to : i >= to; i += dir) {
          const m = _match(perFrame[i].gray, gw, gh, tref, half, pcx, pcy, R, step);
          ...
          if (m.mean > lostThresh) {
            ...
            // keep pcx/pcy/tref unchanged → re-acquire near last good position
          } else {
            pcx = m.x; pcy = m.y;
    ...
    function _buildTrack(perFrame, gw, gh, opts) {
      ...
      for (let i = 0; i < perFrame.length; i++) { const d = Math.abs(perFrame[i].t - atSec); if (d < ab) { ab = d; aIdx = i; } }
      let faceNear = false;
      for (let i = Math.max(0, aIdx - 2); i <= Math.min(perFrame.length - 1, aIdx + 2) && !faceNear; i++) {
        for (const dt of perFrame[i].dets) { if (Math.hypot(dt.cx - sx, dt.cy - sy) <= seedRadius) { faceNear = true; break; } }
      }
      ...
      const backbone = _patchTrack(perFrame, gw, gh, { sx, sy, atSec, patchFrac, searchFrac, step, adapt, lostThresh, recenter });
    ...
    async function analyzeFaceTrack(opts = {}) {
      const { sx = 0.5, sy = 0.5, atSec = null, fps = TRACK.fps, gate = TRACK.gate,
              ...
              lostThresh = TRACK.lostThresh, recenter = TRACK.recenter, onProgress } = opts;
      ...
      log(`tracker[${mode}]: ${perFrame.length} frames, ${detd} with a face, track ${track.length} pts (${t0.toFixed(1)}–${t1.toFixed(1)}s)`);
      return { frames: perFrame.length, detected: detd, points: track.length, mode, t0, t1 };
    }
    ...
    function seedByClick(opts = {}) {
      return new Promise((resolve) => {
        ...
        const host = cv.parentElement;   // .source_embed — the VISIBLE (clipped) frame box
        const onDown = (e) => {
          const r = cv.getBoundingClientRect();          // full canvas box (extends off-screen under the crop) → maps to source coords
          const v = (host || cv).getBoundingClientRect(); // visible frame → gates the click so off-canvas clicks are ignored
    ...
    window.wcTracker = { analyze: analyzeFaceTrack, relink: relinkTrack, seedByClick, marker: trackMarker, boxes: trackBoxes, ensureDetector: ensureFaceDetector };
    let _debug = false;
    let _ready = false;
