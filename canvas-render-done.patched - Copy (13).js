(function () {
  ...
  const _RENDER_SCRIPT_BUILD = 'pb-20260501i-sync';
    ...
    return new Promise((resolve, reject) => {
      ...
      rv.addEventListener('seeked', async function onSeeked() {
        ...
        async function emitFrameAt(index) {
          ...
          if (useMp4) {
            // Pause the source video before awaiting encoder backpressure. Without this,
            // rv races ahead during the await and rVFC catch-up emits duplicate frames
            // (identical canvas state for multiple output timestamps = visible stutter).
            rv.pause();
            if (chatVid) chatVid.pause();
            // Await backpressure signal from Mediabunny — prevents the WebCodecs encoder
            // queue from growing unbounded on slow mobile chips (each queued raw frame is
            // several MB of YUV; a 2-second backlog can OOM the tab).
            await videoSource.add(index / FPS, 1 / FPS);
            // Resume so the next rVFC fires at the correct next-frame position.
