(() => {
  ...
  class Deck {
    ...
    present(ct) {
      ...
      // Opening frame: if the playhead sits BEFORE the earliest decoded frame (e.g. start at output
      // 0 but the first frame's timestamp is a few ms > 0 from an mp4 timestamp offset), show that
      // earliest frame so the preview isn't black at rest. Safe: during normal playback ct is >= the
      // queued frames so the loop above handles it — this only triggers at/just before the start.
      if (!painted && this.presentQ.length && ct <= this.presentQ[0].timestamp) {
        painted = this.presentQ.shift();
      }
      return painted;
    }
