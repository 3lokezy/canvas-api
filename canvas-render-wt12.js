...
if (kickLogo && barEl) {
  ...
  if (bR) {
    badgeLayout = {
      ...
      textStrokeW:     txtEl ? parseFloat(getComputedStyle(txtEl).webkitTextStrokeWidth) * SF_H || 0 : 0,
      textStrokeColor: txtEl ? getComputedStyle(txtEl).webkitTextStrokeColor || 'transparent' : 'transparent',
      behindStrokeW:   (() => {
        const b = [...(barEl.children || [])].find(el => el.classList.contains('behind'));
        return b ? parseFloat(getComputedStyle(b).webkitTextStrokeWidth) * SF_H || 0 : 0;
      })(),
      behindStrokeColor: (() => {
        const b = [...(barEl.children || [])].find(el => el.classList.contains('behind'));
        return b ? getComputedStyle(b).webkitTextStrokeColor || 'transparent' : 'transparent';
      })(),
    };
  }
}
