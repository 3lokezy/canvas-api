(function () {
    ...
    function buildPanelAPI(refs) {
      ...
      window.canvasAPI = {
        ...
        setMusic(url) {
          ...
          _musicAudio.src = url;
          _musicAudio.load();
          document.querySelector('[data-track="music"]')
          ?.classList.remove('is-empty'); 
          _musicAudio.addEventListener('loadedmetadata', () => {
            _musicDuration = _musicAudio.duration;
            window.canvasAPI.setMusicDuration(_musicDuration);
        ...
        setMusicDuration(secs) { 
          _musicDuration = Math.max(1, secs);
          if (tracks.music) {
            tracks.music.end = Math.min(
              tracks.music.start + _musicDuration,
              gpVideo?.duration || _musicDuration
            );
          }
        },
        getMusicDuration()    { return _musicDuration; },
    ...
    function initCanvas(item, opts = {}) {
      ...
      function clampSecondaryTracks() {
        ...
        // ── Cap music track to music file duration ──────────────────────────────
        if (tracks.music && _musicDuration > 0) {
          const maxEnd = tracks.music.start + _musicDuration;
