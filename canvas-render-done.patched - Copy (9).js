(function () {
  ...
  const _RENDER_SCRIPT_BUILD = 'pb-20260501e-cfr';
    ...
    if (_canMP4) {
      if (_useBufferedMp4Audio && typeof AudioEncoder === 'undefined') {
        console.log('Render: WebCodecs H.264 available; no AudioEncoder —',
          _isChromeIOS ? 'video-only MP4 (Chrome iOS)' : 'MediaRecorder audio fallback (Safari iOS)');
      } else {
  ...
  async function renderComposition(onProgress) {
    ...
    // Safari iOS: AudioEncoder absent but MediaRecorder video/mp4 works with audio → use MediaRecorder.
    // Chrome iOS: AudioEncoder absent AND MediaRecorder video/mp4 produces empty output → force
    // Mediabunny for silent-but-postable H.264 MP4.
    const useMp4 = webCodecsMp4Ok &&
      !(_useBufferedMp4Audio && typeof AudioEncoder === 'undefined' && !_isChromeIOS);
    console.log('Render: [3] encoding setup — useMp4:', useMp4, _isIOS ? '(iOS detected)' : '',
      _isChromeIOS ? '(Chrome iOS — video-only MP4)' : '');
    let audioCtx = null;
