(function () {
  ...
  const _useBufferedMp4Audio = _isIOS || _isSafariDesktop;
  ...
  /**
   * Delete every IndexedDB render whose clipId is NOT in allowedIds.
   * Call after you load the user's clips from Supabase so deleted rows drop from local cache.
   */
  async function _pruneCachedRendersExcept(allowedIds) {
    if (!_idbAvailable()) return { removed: [], kept: 0 };
    ...
    try {
      ...
      console.warn('Render: prune cache failed', e);
  ...
  const _WIZED_CLEAR = { url: '', type: '', filename: '', ready: false, thumbUrl: '', thumbFilename: '' };

  // ─────────────────────────────────────────────────────────────────────────────
  // CLOUD UPLOAD — push rendered files to R2 via Cloudflare Worker
  // ─────────────────────────────────────────────────────────────────────────────
  const UPLOAD_WORKER_URL = 'https://render-upload.getstarted-211.workers.dev/upload';
  const UPLOAD_TOKEN      = 'f3b9f6d1-2c84-4b9c-9a41-6d7c9a1e5b72-a8e4c2d7-9f31-4b66-8d2a-1c4f7e9b3a10';                                 // ← match Worker env UPLOAD_TOKEN
  ...
  async function ensureMp4Support() {
    ...
    if (_useBufferedMp4Audio && typeof AudioEncoder === 'undefined') {
      _canMP4 = false;
      console.log('Render: MP4 audio encoder not available, using MediaRecorder fallback');
      return false;
    }
    try {
    ...
    if (_canMP4) console.log('Render: MP4 output enabled (H.264 + AAC)');
    else console.log('Render: MP4 not available, using WebM fallback');
    return _canMP4;
  }
  ...
  async function renderComposition(onProgress) {
    ...
    const useMp4 = await ensureMp4Support();
    ...
    if (useMp4) {
      const { Output, Mp4OutputFormat, BufferTarget,
              CanvasSource, MediaStreamAudioTrackSource, AudioBufferSource: MBAudioBufferSource } = _mb;
      const createVideoSource = () => new CanvasSource(rc, {
        codec: 'avc',
        bitrate: 8_000_000,
      });
      const startTimeoutMs = _useBufferedMp4Audio ? 6000 : 5000;
  ...
  /** Clip id that _lastBlob / cache row belongs to (for prune + clear consistency). */
  let _lastRenderedClipId = null;
  ...
  window.renderAPI = {
    ...
    /**
     * Remove IndexedDB renders for clips that no longer exist in Supabase.
     * Pass the full list of clip ids the user still has (e.g. clips_live rows).
     * @param {string[]} validClipIds — every clip UUID that still exists for this user
     * @returns {Promise<{ removed: string[], kept: number }>}
     */
    async pruneStaleCache(validClipIds) {
      if (!Array.isArray(validClipIds)) {
        console.warn('Render: pruneStaleCache expects an array of clip id strings');
        return { removed: [], kept: 0 };
      }
      const result = await _pruneCachedRendersExcept(validClipIds);
      const allowedSet = new Set(validClipIds.map(String));
      _revokeListUrls();
      if (_lastRenderedClipId && !allowedSet.has(String(_lastRenderedClipId))) {
        if (_lastUrl) URL.revokeObjectURL(_lastUrl);
        if (_lastThumbUrl) URL.revokeObjectURL(_lastThumbUrl);
        _lastBlob = null;
        _lastUrl = null;
      ...
      if (result.removed.length) {
        console.log('Render: pruned stale cache entries', result.removed.length, result.removed);
      }
      try {
        window.dispatchEvent(new CustomEvent('renderCachePruned', { detail: result }));
      } catch (_) {}
      return result;
    },

    /**
     * Upload the current render (video + thumb) to R2 via the Cloudflare Worker.
     * @param {Object} clipData — clip metadata from v.clip_intended.clip
     *   Required: { id, user_id, platform, channel_monitor_id: { id } }
     *   Optional: { filename override }
     * @returns {Promise<{ videoUrl, thumbUrl, renderId }>}
     */
    async uploadToCloud(clipData) {
      if (!clipData || !clipData.id) throw new Error('uploadToCloud: clipData with id is required');

      const videoBlob = _lastBlob;
      const thumbBlob = _lastThumbBlob;
      if (!videoBlob) throw new Error('uploadToCloud: no rendered video available — render first');

      const platform  = clipData.platform || '';
      const userId    = clipData.user_id || '';
      const monitorId = (clipData.channel_monitor_id && typeof clipData.channel_monitor_id === 'object')
        ? clipData.channel_monitor_id.id || ''
        : clipData.channel_monitor_id || '';
      const clipId    = clipData.id;
      const filename  = _lastBlob.type === 'video/mp4'
        ? (clipData._renderFilename || 'render.mp4')
        : (clipData._renderFilename || 'render.webm');

      window.dispatchEvent(new CustomEvent('renderUploadStateChange', { detail: { status: 'uploading', progress: 0 } }));

      const form = new FormData();
      form.append('video', videoBlob, filename);
      if (thumbBlob && thumbBlob.size) form.append('thumb', thumbBlob, 'thumb.jpg');
      form.append('platform', platform);
      form.append('user_id', userId);
      form.append('channel_monitor_id', monitorId);
      ...
      try {
        ...
        console.log('Render: uploaded to R2', result.renderId, result.videoUrl);

        window.dispatchEvent(new CustomEvent('renderUploadStateChange', { detail: { status: 'done', ...result } }));
        return result;
      } catch (e) {
        console.error('Render: upload failed', e);
        window.dispatchEvent(new CustomEvent('renderUploadStateChange', { detail: { status: 'failed', error: e.message } }));
        throw e;
      }
    },
  };
  ...
  window.renderClip = async function () {
    ...
    try {
      ...
      _lastRenderedClipId = clipId;
      _cacheBlob(clipId, { blob, filename, thumbBlob: hasThumb ? thumbBlob : null, thumbFilename });
      _syncToWized({
        url: _lastUrl, type: blob.type, filename, ready: true,
        thumbUrl: _lastThumbUrl || '', thumbFilename,
      });
