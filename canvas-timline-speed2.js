(function () {
  ...
  (function injectStateCSS() {
    ...
    s.textContent =
      ...
      // Slim horizontal zoom scrollbar (was the browser default, too girthy).
      '#timeline_container{scrollbar-width:thin;scrollbar-color:rgba(255,255,255,.28) transparent;}' +
      '#timeline_container::-webkit-scrollbar{height:6px;}' +
      '#timeline_container::-webkit-scrollbar-thumb{background:rgba(255,255,255,.28);border-radius:3px;}' +
      '#timeline_container::-webkit-scrollbar-thumb:hover{background:rgba(255,255,255,.45);}' +
      '#timeline_container::-webkit-scrollbar-track{background:transparent;}';
  ...
  function bindTrackRow(row, api, gpVideo, fullDuration, getWindowOffset, getDisplayDuration, ac) {
    ...
    // Title/image overlays gate in SOURCE time but belong to the OUTPUT cut. The lane must show/trim them in
    // OUTPUT time so the handle lines up with the clips (and with where the title actually hides). We operate
    // the lane in output space and convert at the storage boundary (read: src→out, write: out→src). woff()=0
    // because output is 0-based. Only title/image use this; master(video)+music keep source/window behaviour.
    const OUT_AXIS = (name === 'title' || name === 'image');
    // Title now STORES its window in OUTPUT time (follows reorder), so the lane operates in output space with
    // NO conversion. Image still stores SOURCE time, so it keeps the src↔out conversion below. (Before the
    // output-time migration both converted; converting an already-output title double-mapped it and collapsed
    // the block to zero width — the bug this fixes.)
    const STORE_OUT = (name === 'title');
    const _cl = () => api.getOutputClips?.() || [];
    const _sp = (c) => api?.clipSpeed?.(c) ?? 1;   // per-clip speed folds into src↔output mapping
    const srcToOut = (s) => { const cs = _cl(); for (const c of cs) if (s >= c.sourceStart && s < c.sourceEnd) return c.outputStart + (s - c.sourceStart) / _sp(c); if (cs.length && s <= cs[0].sourceStart) return 0; const l = cs[cs.length - 1]; return l ? l.outputStart + (l.sourceEnd - l.sourceStart) / _sp(l) : s; };
    const outToSrc = (o) => { const cs = _cl(); for (const c of cs) { const oe = c.outputStart + (c.sourceEnd - c.sourceStart) / _sp(c); if (o >= c.outputStart && o < oe) return c.sourceStart + (o - c.outputStart) * _sp(c); } const l = cs[cs.length - 1]; return l ? l.sourceEnd : o; };
    const woff = () => OUT_AXIS ? 0 : getWindowOffset();
    const snapW = (v) => OUT_AXIS ? v : snap(v, api, gpVideo, fullDuration, name);   // clip-edge snap is source-space; skip for output overlays
    ...
    function getWindow() {
      ...
      if (STORE_OUT) {   // title: already stored in output time — no conversion; default end = output duration
        const outDur = api.getOutputDuration?.() ?? getDisplayDuration();
        return { start: t[name]?.start ?? 0, end: t[name]?.end ?? outDur };
      }
      const raw = { start: t[name]?.start ?? 0, end: t[name]?.end ?? fullDuration };
      return OUT_AXIS ? { start: srcToOut(raw.start), end: srcToOut(raw.end) } : raw;   // image: source → output
    }
    let _updateOffsetInner = null;

    function update() {
      const totalW = row.clientWidth;
      const windowOffset = woff();
    ...
    function applyWindow(start, end) {
      if (isMaster) {
        ...
        api.setTitleTiming?.(start, end);   // title: output space IS the storage (no source clamp; fires update)
      } else if (OUT_AXIS) {
        api.setTrack(name, { start: outToSrc(start), end: outToSrc(end) });   // image: output space → source storage
      } else {
    ...
    function bindHandle(handle, side) {
      ...
      handle.addEventListener(
        ...
        (e) => {
          ...
          const snapped = snapW(clampedSec);
          const w = getWindow();
          if (isMaster) {
            ...
            applyWindow(snapped, w.end);            // applyWindow converts output→source for title/image
          } else {
            applyWindow(w.start, snapped);
          }
          update();
        },
        { signal: ac.signal },
    ...
    active.addEventListener(
      ...
      (e) => {
        ...
        if (dragState === 'dragging') {
          ...
          const { effectiveStart, effectiveEnd } = OUT_AXIS
            ? { effectiveStart: 0, effectiveEnd: api.getOutputDuration?.() ?? getDisplayDuration() }
            : api.getTrackBounds();
          ...
          const snappedStart = snapW(newStart);
          const snappedEnd = snapW(newEnd);
  ...
  function mountNLETrack(masterRow, api, getDisplayDuration) {
    ...
    function applyThumb(b) {
      ...
      // CAP the backing resolution so a deep timeline zoom (a very wide bar) can't create a giant
      // canvas — the canvas is CSS width:100% so it still fills the whole bar; past the cap the
      // filmstrip just softens a little instead of exploding memory/paint (was cv.width = bw*dpr,
      // ~27000px at 12× → hundreds of MB, re-alloc per zoom tick + repaint per scroll frame = the lag).
      // Below the cap (all normal/moderate zoom) `lw === bw`, so nothing changes.
      const MAX_THUMB_W = 4000;
      const lw = Math.min(bw, MAX_THUMB_W);
      if (cv.width !== Math.round(lw * dpr) || cv.height !== Math.round(bh * dpr)) {
        cv.width = Math.round(lw * dpr); cv.height = Math.round(bh * dpr);
      }
      ...
      const n  = Math.max(1, Math.round(lw / panelW));
      // Skip the redraw when nothing that affects the pixels changed (capped bars stop re-slicing on
      // every zoom tick once past the cap) — cheap guard, big win while zooming.
      const key = `${lw}|${bh}|${n}|${b.sourceIn}|${b.sourceOut}`;
      if (cv._thumbKey === key) return;
      cv._thumbKey = key;
      const ctx = cv.getContext('2d');
      ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
      ctx.clearRect(0, 0, lw, bh);
      const pw = lw / n;
      for (let i = 0; i < n; i++) {
    ...
    if (initialClips.length > 0) {
      ...
      sorted.forEach((clip, i) => {
        ...
        const cW = Math.max(2, Math.round((clip.sourceEnd - clip.sourceStart) / (api?.clipSpeed?.(clip) ?? 1) / masterDuration * trackW));
    ...
    // RECOMPUTE each bar's left+width straight from the MODEL at the current track width, with a 2px
    // LAYOUT floor (like the initial mount) — NOT a px-from-px rescale with the MIN_W=40 floor.
    // The old path floored every sub-40px bar to a uniform 40px (which happens whenever
    // masterDuration ≫ content — i.e. long, heavily-cut clips → tiny bars), so they all became the
    // same width, overlapped, and drifted with rounding across repeated resizes/zoom. MIN_W now
    // guards ONLY the drag/trim interaction math; zoom is what makes a tiny clip grabbable.
    // Reused by BOTH the ResizeObserver (width changed → scale fallback for unmatched bars) AND
    // discrete model mutations that change bar length without a resize (per-clip SPEED) — those
    // pass no scale (1) since trackW is unchanged and every real bar recomputes from the model.
    function relayoutBars(scale = 1) {
      const byId = new Map((api.getOutputClips?.() || []).map(c => [c.id, c]));
      bars.forEach(b => {
        const c = b.clipId && byId.get(b.clipId);
        if (c && masterDuration > 0) {
          b.currentX = Math.round((c.outputStart / masterDuration) * trackW);
          b.width    = Math.max(2, Math.round(((c.sourceEnd - c.sourceStart) / (api?.clipSpeed?.(c) ?? 1) / masterDuration) * trackW));
        } else {                                        // transient pre-NLE / unmatched bar → proportional, still 2px floor
          b.currentX = Math.round(b.currentX * scale);
          b.width    = Math.max(2, Math.round(b.width * scale));
        }
        b.element.style.left  = `${b.currentX}px`;
    ...
    masterRow._nleRelayout = relayoutBars;
    // Global hook so model-only mutations (setClipSpeed lives in canvas-api, no timeline scope) can
    // repaint the bars without a full remount and without hooking the noisy generic clipsChanged
    // (which fires mid-drag). Mirrors how split/delete trigger _nleRebuild explicitly.
    window._timelineRelayoutBars = () => { try { relayoutBars(); } catch (_) {} };

    const ro = new ResizeObserver(() => {
      const newW = masterRow.clientWidth;
      if (!newW || newW === trackW) return;
      const scale = newW / trackW;   // only for the rare no-model-match fallback inside relayoutBars
      trackW = newW;
      relayoutBars(scale);
    });
  ...
  function _deleteSelectedClip() {
    ...
    if (window.wcImageDeleteSelected?.()) return;        // a selected image cue → remove it (before clip delete)
    const api = window.canvasAPI;
    // A selected MUSIC block → clear the music track (delete has no clip meaning for it; mirrors soundboard).
    const selMusic = document.querySelector('[data-track="music"] .track-active.is-selected');
    if (selMusic && api?.clearMusic) { selMusic.classList.remove('is-selected'); api.clearMusic(); return; }
    const masterRow = document.querySelector('[data-track="video"]');
  ...
  function initTimeline() {
    ...
    const updateFns = rows
      .map((row) => {
        ...
        // 'image' is now the MULTI-image blocks lane (owned by image-track.js, soundboard-style) — skip the
        // single-window trim binder so the two don't fight over the row's .track-active (its clone template).
        if (row.dataset.track === 'image') return null;
    ...
    // Move the playhead element to a fractional position (0..1) along the timeline. Positioned in
    // CONTENT coordinates (offsetLeft, scroll-independent) so that when the timeline is zoomed and
    // #timeline_container scrolls, the playhead — an absolute child of that scroll box — scrolls WITH
    // the content and lands on the right frame. At 1× (no scroll) this equals the old rect-based value.
    function movePlayheadToPct(pct) {
      if (!playhead) return;
      const firstRow = rows[0];
      if (!firstRow) return;
      playhead.style.left = `${firstRow.offsetLeft + pct * firstRow.clientWidth}px`;
    }
    ...
    function syncPlayhead() {
      ...
      if (isNLE) {
        ...
        if (clip) {
          ...
          ot = clip.outputStart + (t - clip.sourceStart) / (api?.clipSpeed?.(clip) ?? 1);
        } else {
    ...
    function rafLoop() {
      ...
      syncPlayhead();
      window._timelineZoomFollow?.();   // when zoomed, scroll to keep the playhead in view
    ...
    // ── Timeline ZOOM (scroll-wheel / pinch) ────────────────────────────────────────────────────
    // Turn #timeline_container into a horizontal scroll box and widen every .timeline-track lane by
    // a zoom factor. The clip-bar ResizeObserver rescales the px bars on the width change; every
    // other lane is %-based so it scales via CSS; the playhead is content-positioned so it scrolls
    // with the content. Floor = 1× (full window — the current view); ceiling = MAX_ZOOM. Raising the
    // floor below 1 later (to zoom OUT past full, once clips can be dragged in) is a one-line change.
    {
      const MAX_ZOOM = 12;
      let _zoom = 1;
      const lanes = () => Array.from(container.querySelectorAll('.timeline-track')).filter(l => l.id !== 'tl_playhead');
      // Reset any width/overflow a previous mount left on the (reused) DOM, so each clip opens at 1×.
      container.style.overflowX = '';
      lanes().forEach(l => { l.style.width = ''; l.style.flexShrink = ''; l.style.maxWidth = ''; });

      function applyWidths() {
        const w = _zoom === 1 ? '' : `${(_zoom * 100).toFixed(2)}%`;
        lanes().forEach(l => {
          l.style.width = w;
          l.style.flexShrink = _zoom === 1 ? '' : '0';   // don't let the flex column shrink a widened lane
          l.style.maxWidth   = _zoom === 1 ? '' : 'none';
        });
      }
      function setZoom(z, anchorClientX) {
        const nz = Math.max(1, Math.min(MAX_ZOOM, z));
        if (Math.abs(nz - _zoom) < 1e-3) return;
        const rect  = container.getBoundingClientRect();
        const viewW = container.clientWidth || rect.width || 1;
        const cx    = anchorClientX != null ? (anchorClientX - rect.left) : viewW / 2;   // pointer x within the viewport
        const frac  = (container.scrollLeft + cx) / (viewW * _zoom);                       // content fraction under the pointer
        _zoom = nz;
        container.style.overflowX = _zoom === 1 ? '' : 'auto';
        applyWidths();
        const newContentW = viewW * _zoom;
        container.scrollLeft = Math.max(0, Math.min(newContentW - viewW, frac * newContentW - cx));   // keep that point put
        syncPlayhead();
      }
      // Plain wheel = zoom (cursor-centred). Shift+wheel or a horizontal trackpad swipe = native pan.
      container.addEventListener('wheel', (e) => {
        if (e.shiftKey || Math.abs(e.deltaX) > Math.abs(e.deltaY)) return;   // let horizontal panning through
        e.preventDefault();
        setZoom(_zoom * (e.deltaY < 0 ? 1.12 : 1 / 1.12), e.clientX);
      }, { signal: ac.signal, passive: false });
      // Pinch (touch): two-finger distance ratio → zoom, centred on the midpoint.
      let pinch = null;
      container.addEventListener('touchstart', (e) => {
        if (e.touches.length === 2) { const [a, b] = e.touches; pinch = { d: Math.hypot(a.clientX - b.clientX, a.clientY - b.clientY), z: _zoom }; }
      }, { signal: ac.signal, passive: true });
      container.addEventListener('touchmove', (e) => {
        if (!pinch || e.touches.length !== 2) return;
        e.preventDefault();
        const [a, b] = e.touches;
        setZoom(pinch.z * (Math.hypot(a.clientX - b.clientX, a.clientY - b.clientY) / pinch.d), (a.clientX + b.clientX) / 2);
      }, { signal: ac.signal, passive: false });
      container.addEventListener('touchend', (e) => { if (e.touches.length < 2) pinch = null; }, { signal: ac.signal, passive: true });
      // Middle-button (scroll-wheel click) drag = PAN, with the browser's middle-click autoscroll
      // suppressed — so panning the zoomed timeline never falls into autoscroll (whose exit-click
      // was landing a seek). Hold the wheel and drag to scroll; release to stop.
      container.addEventListener('mousedown', (e) => { if (e.button === 1) e.preventDefault(); }, { signal: ac.signal });   // kill autoscroll
      let pan = null;
      container.addEventListener('pointerdown', (e) => {
        if (e.button !== 1) return;                       // middle only
        e.preventDefault(); e.stopPropagation();
        pan = { x: e.clientX, sl: container.scrollLeft };
        try { container.setPointerCapture(e.pointerId); } catch (_) {}
        container.style.cursor = 'grabbing';
      }, { signal: ac.signal });
      container.addEventListener('pointermove', (e) => { if (pan) container.scrollLeft = pan.sl - (e.clientX - pan.x); }, { signal: ac.signal });
      const _endPan = () => { if (pan) { pan = null; container.style.cursor = ''; } };
      container.addEventListener('pointerup', _endPan, { signal: ac.signal });
      container.addEventListener('pointercancel', _endPan, { signal: ac.signal });
      container.addEventListener('auxclick', (e) => { if (e.button === 1) e.preventDefault(); }, { signal: ac.signal });
      // Auto-scroll to keep the playing playhead visible (only nudges when it drifts out; never fights a manual scroll).
      window._timelineZoomFollow = () => {
        if (_zoom === 1 || !playhead) return;
        // Only chase the playhead while it's actually PLAYING. The engine stays "active" (so the rAF
        // loop keeps running) even when paused — without this gate the follow fired every frame while
        // paused and yanked scrollLeft back to the playhead, fighting a manual scroll (the glitch).
        const eng = window.wcEngine;
        const playing = eng?.isActive?.() ? !eng.paused : (!gpVideo.paused && !gpVideo.ended);
        if (!playing) return;
        const viewW = container.clientWidth, x = parseFloat(playhead.style.left) || 0, margin = viewW * 0.1;
        if (x < container.scrollLeft + margin)            container.scrollLeft = Math.max(0, x - margin);
        else if (x > container.scrollLeft + viewW - margin) container.scrollLeft = x - viewW + margin;
      };
      // Programmatic hooks (a Fit / ± button can call these).
      api.setTimelineZoom = (z) => setZoom(z, null);
      api.getTimelineZoom = () => _zoom;
      api.resetTimelineZoom = () => setZoom(1, null);
      // Fit the CONTENT (the clipped-down length) to the viewport: zoom so the kept clips fill the
      // view and scroll to the start (the dead-space tail past the content scrolls off to the right).
      // Used after an AI edit trims a LONG source (>90s ruler) down — the clips would otherwise sit in
      // a small slice of the fixed ruler. No-op for short sources or when nothing was clipped down.
      api.fitTimelineZoom = () => {
        const md = api._nleTimelineDuration || 0;
        const content = api.getOutputDuration?.() || 0;
        if (md <= 90 || !(content > 0) || content >= md * 0.95) return;
        setZoom(md / content, null);
