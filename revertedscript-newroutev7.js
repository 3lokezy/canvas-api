    ...
    function initCanvas(item, opts = {}) {
      ...
      function restoreModeZoomPan(mode) {
        ...
        const doRestore = () => {
          ...
          gpVideo.style.width  = `${dw}px`;
          gpVideo.style.height = `${dh}px`;
          gpVideo.style.left   = `${Math.max(hw - dw, Math.min(0, centeredL + saved.panX * hw))}px`;
          gpVideo.style.top    = `${Math.max(hh - dh, Math.min(0, centeredT + saved.panY * hh))}px`;
        };
        ...
        gpVideo.addEventListener('loadedmetadata', () => {
          ...
          if (saved?.zoom) {
            ...
            if (gpW && gpH) {
                const cL = (gpHW - gpW) / 2;
                const cT = (gpHH - gpH) / 2;
                gpVideo.style.left = `${Math.max(gpHW - gpW, Math.min(0, cL + saved.panX * gpHW))}px`;
                gpVideo.style.top  = `${Math.max(gpHH - gpH, Math.min(0, cT + saved.panY * gpHH))}px`;
            }
      ...
      function applyModeFn(mode) {
        ...
        if (leavingMode && leavingMode !== mode &&
          ...
          const cL = (gpHW - gpVideo.offsetWidth)  / 2;
            const cT = (gpHH - gpVideo.offsetHeight) / 2;
            modeStates[leavingMode] = { zoom: gpZoom, panX: (gpLeft - cL) / gpHW, panY: (gpTop - cT) / gpHH };
        }
