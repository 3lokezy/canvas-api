  ...
  async function renderComposition(onProgress) {
    ...
    if (useMp4) {
      ...
      setupMp4AudioSource = async () => {
        ...
        try {
          ...
          // Pass the ArrayBuffer directly — decodeAudioData detaches it, freeing
          // raw bytes immediately. Never .slice(0) here: copying 100–300 MB causes
    ...
    return new Promise((resolve, reject) => {
      ...
      rv.addEventListener('seeked', async function onSeeked() {
        ...
        // timestamps (index/FPS). rv is force-synced to each frame's exact
        // timestamp in pumpEmits so pixel content always matches.
        const totalFrames  = Math.max(1, Math.round(totalDuration * FPS));
        let nextEmitIndex  = 0;
        let renderFinished = false;

        // iOS: seek rv to each frame rather than play/pause cycling.
        // play() after a programmatic pause is unreliable on iOS Safari —
        // it stalls silently, rVFC fails to re-arm, and remaining frames
        // pad as frozen duplicates with audio desync.
        ...
        async function pumpEmits(currentMediaTime) {
          ...
          while (nextEmitIndex < totalFrames && (nextEmitIndex / FPS) <= offset + 1e-6) {
            ...
            // Force-sync rv to the exact output timestamp. Without this, rv can
            // drift ahead of the declared frame time during the pause/play cycle,
            // causing pixel content to lead its timestamp.
        ...
        if (_iosSeekMode) {
          ...
          rv.pause();
          if (chatVid) chatVid.pause();
          console.log('Render: [iOS] seek loop — totalFrames:', totalFrames);
          (async () => {
            try {
