...
if (kickLogo && barEl) {
  ...
  if (bR) {
    ...
    const useMp4 = false; // TODO: re-enable Mediabunny when social posting requires MP4
    ...
    if (useMp4) {
      ...
      try {
        ...
        if (isInSource) {
          // Use captureStream → createMediaStreamSource to avoid cross-origin CORS silent-fail.
          // createMediaElementSource on cross-origin video creates the node without throwing
          // but outputs silence — captureStream has no CORS restriction.
      ...
      rv.addEventListener('seeked', async function onSeeked() {
        ...
        try {
          ...
          if (!(useMp4 && mp4AudioBuffer)) {
            rv.muted  = false;
            rv.volume = 0; // silence speakers — audio still captured via Web Audio graph
          }
          if (audioCtx?.state === 'suspended') {
            await Promise.race([audioCtx.resume(), new Promise(r => setTimeout(r, 500))]);
          }
