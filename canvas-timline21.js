  ...
  function initTimeline() {
    ...
    function syncPlayhead() {
      ...
      if (eng?.isActive?.()) {
        ...
        const ruler    = api.getTimelineDuration?.() || api._nleTimelineDuration || api.getOutputDuration?.() || 0;
    ...
    // Click anywhere that isn't a clip/track block (or a tool button) → deselect (unfocus).
    document.addEventListener('pointerdown', (e) => {
      if (e.target.closest?.('.track-active')) return;
    ...
    (function ensureNLE() {
      ...
      if (!_restoredOnce && _nleTries++ < 33) { setTimeout(ensureNLE, 150); return; }
