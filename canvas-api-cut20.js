(function () {
  ...
  function buildPanelAPI(refs) {
    ...
    window.canvasAPI = {
      ...
      // ONE source of truth for the timeline length, shared by BOTH the timeline
      // render (canvas-timeline.js) and the playback engine (canvas-webcodecs.js) so
      // they can never diverge into two behaviours.
      //   COLLAPSE semantics: it tracks the ACTUAL clip content (getOutputDuration).
      //   Deletes ripple (clips pack contiguously), so the timeline shrinks to fit
      //   and the playhead stops exactly at the last clip — it can't run past the end.
      // Falls back to the fixed editable region (VOD window / live trim region) only
      // before any clips exist yet.
      getTimelineDuration() {
        const d = this.getOutputDuration();
        if (d > 0) return d;
