  ...
  function initCanvas(item, opts = {}) {
    ...
    if (splitHandle && gpHold && fcHold) {
      ...
      applySplit = pct => {
        ...
        // over the facecam (and the face). Set ONLY `top` — leave the bar's own
        // transform intact (it carries the horizontal centering; overriding it pushed
        // the bar off-frame → invisible). % is relative to the full-frame
        // .kick_twitch_overlay → maps 1:1 to the frame (resize-safe). Covers all
        // platform variants (kick/twitch/hidden) that share the class.
        frame.querySelectorAll('.kt_overlay_placement_bar').forEach(_bar => {
