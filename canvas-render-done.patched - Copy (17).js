(function () {
  ...
  const _RENDER_SCRIPT_BUILD = 'pb-20260502b-ios-remux-stream';
  ...
  async function ensureMp4Support() {
    ...
    try {
      [_mb, _mbMp4] = await Promise.all([
        import('https://esm.sh/mediabunny@1.42.0'),
        import('https://esm.sh/mp4box@0.5.2'),
      ]);
  ...
  // Uses streaming + AbortController so we stop downloading as soon as we
  // have all audio samples up to endSec — no need to read the whole file.
  async function extractRawMp4Audio(sourceUrl, startSec, endSec, signal) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      file.onReady = info => {
        ...
        try {
          desc = file.getTrackById(track.id).mdia.minf.stbl.stsd.entries[0];
        } catch (_) {}
      ...
      file.onSamples = (id, _, batch) => {
        ...
        // Last sample in batch is past endSec → we have everything; flush early.
        const last = batch[batch.length - 1];
        if (last && last.dts >= s1 && samples.length > 0) file.flush();
      };

      file.onFlush = () => settle(track && samples.length ? { track, desc, samples } : null);
      file.onError = e => { if (!settled) reject(new Error(String(e))); };

      if (signal?.aborted) { settle(null); return; }

      try {
        const res = await fetch(sourceUrl, signal ? { signal } : {});
        if (!res.ok) { settle(null); return; }

        if (res.body && typeof res.body.getReader === 'function') {
          // Streaming path — abort download as soon as onFlush fires (early settle).
          const reader = res.body.getReader();
          while (!settled) {
            const { done, value } = await reader.read();
            if (done) { if (!settled) file.flush(); break; }
            // Copy chunk — .buffer may be a shared backing store on some engines.
          ...
          // Fallback for browsers without ReadableStream body (iOS < 14.1).
          const ab = _toMp4BoxBuf(await res.arrayBuffer());
          if (!settled) { file.appendBuffer(ab); file.flush(); }
        }
      } catch (e) {
        if (e.name === 'AbortError') { settle(null); return; }
        if (!settled) reject(e);
      }
    });
  }
