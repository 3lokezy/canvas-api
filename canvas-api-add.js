    ...
    function buildPanelAPI(refs) {
      ...
      window.canvasAPI = {
        ...
        setTitleStyle(styleId) {
            ...
            if (!pill) return;
            // Remove any existing style combo class
            pill.classList.forEach(cls => {
              if (/^\d{3}$/.test(cls)) pill.classList.remove(cls);
            });
            if (styleId) pill.classList.add(styleId);
            // Re-render pills so clones inherit the updated class
        ...
        getState() {
          ...
          return {
            ...
            titleStyle: (() => {
                const pill = titleHold?.querySelector('.title_pill');
                return Array.from(pill?.classList ?? []).find(cls => /^\d{3}$/.test(cls)) || null;
              })(),
              subtitleStyle: (() => {
                const pill = document.querySelector('.subtitle_pill');
                return Array.from(pill?.classList ?? []).find(cls => /^\d{3}$/.test(cls)) || null;
