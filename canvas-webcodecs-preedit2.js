  ...
  function createEngine(gpVideo, cv, media) {
    ...
    function tick() {
      ...
      if (!clip) {
        ...
        if (main) { main.extendTo(srcUs); const f = main.present(srcUs); if (f) {
          if (armThrottle === 0) console.log('[wc] vlag', +((srcUs - f.timestamp)/1e6).toFixed(3), 's | qlen', main.presentQ.length, '| decQ', main.decoder?.decodeQueueSize);
          draw(f); f.close(); } }
