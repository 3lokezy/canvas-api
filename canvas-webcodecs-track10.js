(() => {
  ...
  async function enable() {
    ...
    syncBtns();
    // First-frame paint: enabling while PAUSED, the initial prime→present races the clips/canvas/
    // mode setup and can leave the opening frame black until the user seeks (confirmed: a manual
    // seekOutput paints it). Nudge a reseek to the current position once the scene has settled —
    // seekOutput sets _needReseek, so the next tick re-primes and present() draws the frame.
    if (engine.paused) {
      const _paintFirst = () => { try { if (state.active && engine.paused) engine.seekOutput(engine.currentOutputTime()); } catch (_) {} };
      requestAnimationFrame(_paintFirst);
      setTimeout(_paintFirst, 300);
    }
