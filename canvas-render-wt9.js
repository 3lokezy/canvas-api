(function () {
  ...
  async function renderComposition(onProgress) {
    ...
    const wmVid     = (wmSrc && state.watermarkVisible !== false)
      ? await loadVideo(wmSrc).catch(() => null)
      : null;
      if (wmVid) {
        wmVid.loop = true;
        await new Promise(res => {
          const t = setTimeout(res, 2000);
          wmVid.addEventListener('seeked', () => { clearTimeout(t); res(); }, { once: true });
          wmVid.currentTime = 0.001;
        });
      }

// ── Source badge ──────────────────────────────────────────────────────────
const KICK_LOGO_URL = 'https://cdn.prod.website-files.com/69933f20a795f0823811a081/6a105cf53947faa131f9da65_kick%20svg.svg'
const badgeUsername = document.querySelector('[wized="stream_source_username"]')?.textContent.trim() || '';
const kickLogo      = (state.sourceBadgeVisible !== false && badgeUsername)
  ? await loadImage(KICK_LOGO_URL).catch(() => null)
  : null;
...
if (kickLogo) {
  ...
  const barEl = document.querySelector('#source_badge div');
  const imgEl = document.querySelector('#source_badge img');
  const txtEl = document.querySelector('#source_badge [wized="stream_source_username"]')
  || document.querySelector('#source_badge div > *:last-child');
  if (barEl) {
    ...
    badgeLayout = {
      ...
      barH:    bR.height * SF_H,
      barBg:   getComputedStyle(barEl).backgroundColor || '#000000',
      ...
      textSize:   tR ? parseFloat(getComputedStyle(txtEl).fontSize) * SF_H : 28,
      textWeight: txtEl ? getComputedStyle(txtEl).fontWeight : '700',
      textFamily: txtEl ? getComputedStyle(txtEl).fontFamily : 'Inter, sans-serif',
      textColor:  txtEl ? getComputedStyle(txtEl).color : '#ffffff',
    };
  }
}
    ...
    const useMp4      = webCodecsMp4Ok && (typeof AudioEncoder !== 'undefined' || _isIOS || _isChromeIOS);
...
if (kickLogo && badgeLayout) {
  ...
  ctx.textAlign = 'right';
  ctx.fillText(`kick.com/${badgeUsername}`, badgeLayout.textX, badgeLayout.textY);
  ctx.restore();
} 
        }
