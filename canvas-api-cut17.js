    ...
    function _syncMusicToVideo() {
      ...
      if (drift > 0.2) {
        if (window.WC_MUSIC_DIAG) {
          const out = _wcOn() ? window.wcEngine.currentOutputTime() : null;
          console.log(`[music] seek ${_musicAudio.currentTime.toFixed(2)}→${targetTime.toFixed(2)} ` +
            `drift=${drift.toFixed(2)}  srcT=${t.toFixed(2)}  outT=${out == null ? '-' : out.toFixed(2)}  ` +
            `trackStart=${trackStart.toFixed(2)} offset=${_musicOffset.toFixed(2)}`);
        }
