(function () {
  ...
  // Delete the SELECTED clip (ripple — deleteClip closes the gap). The select-then-Delete
  // convention; no tool mode. After the rebuild we re-select the neighbour (the next clip,
  // or the previous if we deleted the last) so a sequence can be cleared with repeated
  // presses and the panel keeps an edit target. No-op when nothing's selected or only one
  // clip remains (deleteClip itself guards the last clip too).
  function _deleteSelectedClip() {
    const api = window.canvasAPI;
    const masterRow = document.querySelector('[data-track="video"]');
    if (!api || !masterRow?._nleReady) return;
    const id = api.getEditClip?.();
    if (!id) return;                                   // nothing selected → nothing to delete
  ...
  function bindScissorTool(masterRow, api, getWindowOffset, getDisplayDuration, ac) {
    ...
    // One-press split at the playhead (no sticky scissor mode). The legacy click-anywhere
    // scissor path below stays dormant — _toolMode is never set to 'scissor' anymore.
  ...
  function bindDeleteTool(masterRow, api, getWindowOffset, getDisplayDuration, ac) {
    ...
    // One-press delete of the selected clip (no sticky delete mode). The legacy
    // click-a-clip-in-delete-mode path below stays dormant — _toolMode is never 'delete'.
    btn.addEventListener('click', () => {
      _deleteSelectedClip();
    }, { signal: ac.signal });
