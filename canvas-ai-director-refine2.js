(function () {
    ...
    for (const t of work) {
      ...
      for (let i = 0; i < bounds.length - 1; i++) {
        ...
        if (i === reactIdx && reactOn) {
          ...
          s = solo ? (camShot ? { from: a, to: b, mode: 'is-full', person: camP, zoom: 1.6 }
  ...
  function applyIntents(plan) {
    ...
    // TIME RESOLUTION: the model reasons in SOURCE time (segments, analysis, storyboard cells are all source),
    // so its sfx/image cue times are SOURCE too — plus an optional `anchor` (storyboard cell id) for a beat it
    // can see. The CLIENT converts source→output via the kept ranges (measured; handles trims + reorders) so the
    // model never does error-prone output arithmetic. CONFIRMED empirically: cues arrive at source seconds (e.g.
    // sfx at 49.5s inside kept range 48.8–51.4) and were being placed RAW as output → each earlier cut pushed
    // them later, some past the end (never fired). Converting here is the alignment fix.
    try {
      ...
      const srcToOut = st => {
        ...
        return null;   // source moment landed on CUT content → not in the final cut
      };
      // a cue's SOURCE moment = its anchor cell's time if given, else its numeric time (read as SOURCE now)
      const srcMoment = (c, key) => {
        if (c.anchor) { const cell = cells.find(x => x.cell === String(c.anchor).toUpperCase().trim()); if (cell) return cell.t; }
      ...
      const resolve = (arr, kind) => {
        ...
        for (const c of arr) {
          if (!c) continue;
          const src = srcMoment(c, kind === 'at' ? 'at' : 'from');
          if (src == null) { out.push(c); continue; }               // no usable time — leave for the handler to clamp
          const ot = srcToOut(src);
          if (ot == null) { console.log('[ai-polish] cue dropped (on cut content) src ' + src.toFixed(1) + 's' + (c.anchor ? ' [' + c.anchor + ']' : '')); continue; }
          if (kind === 'at') c.at = +ot.toFixed(2);
          else { const dur = Math.max(0.5, (+c.to - +c.from) || 2); c.from = +ot.toFixed(2); c.to = +(ot + dur).toFixed(2); }   // window length: source≈output within a scene
          console.log('[ai-polish] cue ' + (c.anchor ? c.anchor + ' ' : '') + 'src ' + src.toFixed(1) + 's → output ' + ot.toFixed(1) + 's');
          out.push(c);
        }
        return out;
      };
      det.sfx    = resolve(det.sfx, 'at');
      det.images = resolve(det.images, 'range');
    } catch (_) {}
