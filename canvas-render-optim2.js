...
if (kickLogo && barEl) {
  ...
  if (bR) {
    ...
    let transcript = null;
    let subChunks  = null;
    let subEmph    = new Set();   // salient-word start times (shared predicate → parity with the live preview)
    const CHUNK_SIZE = 3;
    ...
    try {
      ...
      if (raw) {
        ...
        if (Array.isArray(parsed) && parsed.length) {
          ...
          if (window._capEmphWords) subEmph = window._capEmphWords(transcript);
          if (subMode === 'chunk') {
            subChunks = [];
            ...
            if (transcript && state.subtitleVisible) {
              ...
              if (subMode === 'chunk' && subChunks) {
                ...
                if (foundChunk !== -1) {
                  ...
                  renderSubChunkAtY(ctx, subChunks[foundChunk], foundWord, subY, ts, _subAnim(state.subtitleStyle, ms - aw.start), subEmph);
                }
              } else {
                const word = transcript.find(w => ms >= w.start && ms < w.end);
                if (word) renderSubWordAtY(ctx, word.text, subY, ts, _subAnim(state.subtitleStyle, ms - word.start), subEmph.has(word.start));
              }
            }
          }
  ...
  // Caption keyword emphasis predicate (window._capIsEmph) is defined at load in canvas-api.js so both the
  // live-preview DOM and this canvas export agree on which words pop. Used in renderSubWord/Chunk below.
  ...
  function renderSubWordAtY(ctx, text, y, ts, anim, emph) {
    const eScale = emph ? (window._capEmphCfg?.scale || 1.14) : 1;   // salient word → colour + scale pop
    ...
    const aScale = (anim ? anim.scale : 1) * eScale;
    const aDy    = anim ? (anim.dy || 0) : 0;
    ctx.save();
    if (aScale !== 1 || aDy) {                                   // entry motion × emphasis pop, about centre
      ctx.translate(cx, cy + aDy * ts.sFontSize);
    ...
    drawStyledText(ctx, text, OUT_W / 2, y + ph / 2,
      emph ? (window._capEmphCfg?.color || '#ffe14d') : ts.sActiveColor, ts.sActiveTextShadow,
      ts.sActiveStrokeWidth, ts.sActiveStrokeColor);
    ctx.restore();
    ctx.letterSpacing = '0px';
  }

  function renderSubChunkAtY(ctx, chunk, activeIdx, y, ts, anim, emphSet) {
    const font = `${ts.sFontWeight} ${ts.sFontSize}px ${ts.sFontFamily}`;
    ctx.font         = font;
    ctx.textBaseline = 'middle';
    if (ts.sLetterSpacing) ctx.letterSpacing = `${ts.sLetterSpacing}px`;
    ...
    for (let i = 0; i < metrics.length; i++) {
      ...
      const emph     = emphSet && emphSet.has(chunk[i].start);   // colour-only in chunk mode (scale would shift the row layout)
      ...
      drawStyledText(ctx, m.text, cx, cy,
        emph ? (window._capEmphCfg?.color || '#ffe14d') : (isActive ? ts.sActiveColor : ts.sColor),
