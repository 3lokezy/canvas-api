(function () {
  ...
  const _RENDER_SCRIPT_BUILD = 'pb-20260502h-last-frame-fix';
  ...
  async function remuxWithAudio(videoBuffer, audioData) {
    ...
    const { vTrack, vDesc, vSamples } = await new Promise((resolve, reject) => {
      ...
      file.onReady = info => {
        ...
        if (!vTrack) { reject(new Error('remuxWithAudio: no video track in Mediabunny output')); return; }
        try {
      ...
      file.onSamples = (id, _, batch) => { vSamples.push(...batch); };
      file.onFlush  = () => resolve({ vTrack, vDesc, vSamples });
    ...
    if (useMp4) {
      ...
      setupMp4AudioSource = async () => {
        ...
        if (typeof AudioEncoder === 'undefined') {
          ...
          if (_isIOS && _mbMp4 && !_rawAudioPromise) {
