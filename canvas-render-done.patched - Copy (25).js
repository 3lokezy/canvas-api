(function () {
  ...
  const _RENDER_SCRIPT_BUILD = 'pb-20260502j-ios-audio-remux';
  ...
  async function remuxWithAudio(videoBuffer, audioData) {
    ...
    for (const s of vSamples) {
      outFile.addSample(videoId, s.data, {
        ...
        is_sync: s.is_sync, description: s.description ?? vDesc,
      });
    }
    ...
    for (const s of audioData.samples) {
      outFile.addSample(audioId, s.data, {
        duration: s.duration, dts: s.dts, cts: s.cts ?? s.dts,
        is_sync: s.is_sync, description: s.description ?? audioData.desc,
      });
    }
