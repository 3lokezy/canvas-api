(function () {
  ...
  // Manual PER-CLIP crop repositioning: drag the split bottom panel to pan which region of the source it
  // shows, committed to the edit clip's `override.facecam` (so different clips can frame different people/cams
  // by hand — the same field the AI sets). Pan-only for Stage 1; zoom/resize within the panel = Stage 2.
  function bindSplitCropDrag(fcHold, item) {
    ...
    fcHold.addEventListener('pointerdown', (e) => {
      ...
      const cur = cid && api.getClipFacecam?.(cid);
      if (!cid || !cur) return;
      e.preventDefault(); e.stopPropagation();
      const rect = fcHold.getBoundingClientRect();
      drag = { cid, cur, startX: e.clientX, startY: e.clientY, w: rect.width || 1, h: rect.height || 1, box: { ...cur } };
      try { fcHold.setPointerCapture(e.pointerId); } catch (_) {}
      fcHold.style.cursor = 'grabbing';
    });
    fcHold.addEventListener('pointermove', (e) => {
      if (!drag) return;
    ...
    const end = () => {
      if (!drag) return;
      window.canvasAPI?.setClipFacecam?.(drag.cid, drag.box);     // commit → per-clip override
      drag = null; fcHold.style.cursor = '';
    };
    fcHold.addEventListener('pointerup', end);
    fcHold.addEventListener('pointercancel', end);
