  ...
  async function renderComposition(onProgress) {
    ...
    const wmVid     = (wmSrc && state.watermarkVisible !== false)
      ...
      if (wmVid) {
        ...
        wmVid.pause();
          ...
          // ── Source badge ──────────────────────────────────────────────────────────
    const KICK_LOGO_URL  = 'https://pub-ede1a356e8044cfc886398da0558ac9a.r2.dev/kick%20svg.svg';
    ...
    return new Promise((resolve, reject) => {
      ...
      rv.addEventListener('seeked', async function onSeeked() {
        ...
        if (kickLogo && state.sourceBadgeVisible !== false) {
          const BAR_H   = Math.round(OUT_H * 0.07);
          const BAR_Y   = Math.round(OUT_H * 0.87);
          const PAD     = Math.round(OUT_W * 0.05);
          const LOGO_H  = Math.round(BAR_H * 0.45);
          const LOGO_W  = Math.round(LOGO_H * (kickLogo.naturalWidth / kickLogo.naturalHeight));
          const LOGO_Y  = BAR_Y + (BAR_H - LOGO_H) / 2;
          const TEXT_SIZE = Math.round(BAR_H * 0.35);

          ctx.save();
          ctx.fillStyle = '#000000';
          ctx.fillRect(0, BAR_Y, OUT_W, BAR_H);
          ctx.drawImage(kickLogo, PAD, LOGO_Y, LOGO_W, LOGO_H);
          ctx.fillStyle = '#ffffff';
          ctx.font = `700 ${TEXT_SIZE}px "Inter", sans-serif`;
          ctx.textBaseline = 'middle';
          ctx.textAlign = 'right';
          ctx.fillText(`kick.com/${badgeUsername}`, OUT_W - PAD, BAR_Y + BAR_H / 2);
          ctx.restore();
        }
        }
