(function () {
  ...
  // Cascade / parent-chain inheritance. A clip's effective value for `key` (layout | mode)
  // is taken from the nearest clip AT-OR-BEFORE it in timeline (output) order that carries an
  // explicit `override[key]`. So editing a clip makes it the "parent" for everything downstream
  // until the next clip with its own override; resetting that clip lets the range fall through to
  // the next upstream parent. Returns undefined when nothing upstream overrides → caller uses base.
  // (Tracking is intentionally NOT cascaded — a motion path is keyed to specific source frames.)
  ...
  function buildPanelAPI(refs) {
    ...
    window.canvasAPI = {
      ...
      // Mode the UI should show as active: the SELECTED clip's effective mode (override ?? base),
      // else base. Lets the mode buttons reflect the clip you're parked on as you click between them.
      getEffectiveMode() {
        const id = this.getEditClip();
        if (id) {
          const clips = tracks.video.clips || [];
          const m = _cascadeOverride(clips, clips.find(c => c.id === id), 'mode');   // nearest upstream parent's mode
          if (m) return m;
        }
        return frame._baseMode || 'is-full';
      },
      // Effective-mode runs across the timeline (cascade-resolved, then merged): consecutive clips
      // sharing an effective mode collapse into one block; the run splits wherever the mode changes.
      // Output-time keyed so the mode lane lines up under the clip track and follows cuts/reorder/
      // trims. Returns [{ mode, outputStart, outputEnd, clips }] in timeline order.
      getModeSegments() {
        ...
        for (const c of ordered) {
          ...
          if (!(span > 0)) continue;
          const mode  = _cascadeOverride(all, c, 'mode') || base;
        ...
        try {
          ...
          blob.arrayBuffer()
            .then(ab => {
              ...
              if (!C) return null;
              return new C(1, 1, 44100).decodeAudioData(ab.slice(0));
            })
      ...
      getEditClip() {
        return document.querySelector('.track-active.is-selected[data-clip-id]')?.dataset.clipId || null;
      },
      ...
      // Per-clip, per-character track store → toggling persists across reload (analyze once per
      // character per clip, ever). `charTracks` rides on the clip override (serialized + restored).
      setCharacterTrack(charId, raw) {
        const editClip = this.getEditClip();
        if (!editClip || !raw || !raw.length) {
          // No clip selected → editing base. Cache the track ON THE CHARACTER so a later click
          // re-applies instantly instead of re-analyzing, and drive the live (base) reframe.
          if (raw && raw.length) { const ch = _characters.find(c => c.id === charId); if (ch) ch.track = raw; _baseCharacter = charId; }
          if (window.wcReframe) { window.wcReframe.setTrack(raw || []); window.wcReframe.enable(!!(raw && raw.length)); }
          window.dispatchEvent(new CustomEvent('charactersChanged'));
          return;
        }
        const clips = (tracks.video.clips || []).map(c => {
          if (c.id !== editClip) return c;
          const charTracks = { ...(c.override?.charTracks || {}), [charId]: raw };
          return { ...c, override: { ...(c.override || {}), charTracks, tracking: { enabled: true, raw, mode: 'face' }, character: charId } };
        });
        this.setClips(clips);
        window.dispatchEvent(new CustomEvent('charactersChanged'));
      },
      selectCharacterTrack(charId) {   // activate a stored character track instantly; false if not yet analyzed for this clip
        const editClip = this.getEditClip();
        if (!editClip) {
          // Base editing → use the character's own cached track if it has one.
          const ch = _characters.find(c => c.id === charId);
          if (!ch || !ch.track || !ch.track.length) return false;
          _baseCharacter = charId;
          if (window.wcReframe) { window.wcReframe.setTrack(ch.track); window.wcReframe.enable(true); }
          window.dispatchEvent(new CustomEvent('charactersChanged'));
          return true;
        }
        const raw = (tracks.video.clips || []).find(c => c.id === editClip)?.override?.charTracks?.[charId];
      ...
      // Wipe a character's tracking data for the edit clip (keeps the character in the library). If it
      // was the active one, turn tracking off.
      clearCharacterTrack(charId) {
        const editClip = this.getEditClip();
        if (!editClip) {   // base editing → wipe the character's base-cached track
        ...
        const clips = (tracks.video.clips || []).map(c => {
          if (c.id !== editClip || !c.override) return c;
          const ct = { ...(c.override.charTracks || {}) }; delete ct[charId];
          const ovr = { ...c.override, charTracks: Object.keys(ct).length ? ct : undefined };
          if (ovr.character === charId) { ovr.character = undefined; ovr.tracking = { enabled: false }; }
          return { ...c, override: ovr };
        });
        this.setClips(clips, false);
        window.dispatchEvent(new CustomEvent('charactersChanged'));
      },
      ...
      getState() {
        ...
        const gpHH   = gpHold?.clientHeight ?? 1;
        return {
          mode:            frame._baseMode ?? frame._currentMode ?? 'is-full',   // BASE mode (what clips inherit), not the displayed clip's effective mode — restore sets base, cascade re-derives per-clip
  ...
  function initCanvas(item, opts = {}) {
    ...
    bindTrack(gpVideo, imgHold,   () => tracks.image);
      ...
      function _applyReframe(srcSec, ot, clip) {
        ...
        if (clip !== undefined) {
          ...
          if (cid !== _lastLayoutClipId) {
            ...
            // MODE: nearest upstream override (cascade) else base. Switch only on an actual change
            // (reuses the tested inner applyMode — DOM swap WITHOUT touching the global modeStates).
            const effMode = _cascadeOverride(tracks.video.clips, clip, 'mode') || frame._baseMode;
