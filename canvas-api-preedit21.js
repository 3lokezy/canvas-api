(function () {
  ...
  function buildPanelAPI(refs) {
    ...
    window.canvasAPI = {
      ...
      // Images may overlap (no neighbour truncation); effDur is clamped so a cue can't run past content end.
      getImageCues() {
        ...
        return _images
          .map(im => {
            const outputStart = Math.max(0, im.at);
            const effDur = Math.max(0.1, Math.min(im.dur, isFinite(cap) ? cap - outputStart : im.dur));
            return { id: im.id, url: im.url, name: im.name || '', outputStart, outputEnd: outputStart + effDur, effDur, x: im.x, y: im.y, scale: im.scale };
          })
          .filter(c => c.effDur > 0.05 && c.outputStart < cap)
          .sort((a, b) => a.outputStart - b.outputStart);
      },
  ...
  function initCanvas(item, opts = {}) {
    ...
    (function bindImageCues() {
      ...
      let _geomWarned = false;
      const apply = (ot) => {
        const { cues, cue } = pick(ot);
        if (!cues.length) return;                                   // legacy path owns the overlay
        if (!cue) {
          if (_shownId !== null) { imgHold.style.opacity = '0'; imgHold.style.visibility = 'hidden'; _shownId = null; imgHold._activeCueId = null; }
          return;
        }
        // Set tracking + visibility FIRST — these can't throw, so the hide/swap path stays correct even if the
        // geometry helpers below throw. (wcTimeLayers wraps each layer in a silent try/catch, so a throw there
        // would otherwise leave _shownId null → the `!cue` hide branch never fires → the image never clears.)
        _shownId = cue.id;
        imgHold._activeCueId = cue.id;
        imgHold._userVisible = true;
        if (imgEl.getAttribute('src') !== cue.url) imgEl.src = cue.url;
        imgHold.style.opacity = '1';
        imgHold.style.visibility = '';
        if (!imgHold._imgDragging) {                                 // drag owns the DOM mid-gesture
          try { applyImgScale(cue.scale); applyImgPosition(cue.x, cue.y); }
          catch (e) { if (!_geomWarned) { _geomWarned = true; console.warn('[image] geometry apply failed', e); } }
        }
      };
      (window.wcTimeLayers ||= []).push((srcSec, ot) => apply(ot != null ? ot : srcSec));
