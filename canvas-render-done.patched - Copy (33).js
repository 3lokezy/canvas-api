(function () {
  ...
  const _RENDER_SCRIPT_BUILD = 'pb-20260505s-timer-seek';
  ...
  async function renderComposition(onProgress) {
    ...
    return new Promise((resolve, reject) => {
      ...
      rv.addEventListener('seeked', async function onSeeked() {
        ...
        // Set true when rVFC watchdog falls back to timer loop — disables
        // rv.play()/rv.pause() cycling and uses per-frame seeks instead,
        // so the loop is immune to external code that re-pauses rv.
        ...
        async function emitFrameAt(index) {
          ...
          if (useMp4) {
            ...
            if (!_iosSeekMode && !_timerSeekMode) {
              rv.pause();
              if (chatVid) chatVid.pause();
            }
        ...
        function startTimerLoop() {
          // Seek-based sequential loop — immune to external code re-pausing rv.
          // rVFC failed (video off-compositor), so we seek rv to each frame's
          // timestamp instead of relying on rv.currentTime advancing via play().
          _timerSeekMode = true;
          rv.pause();
          if (chatVid) chatVid.pause();
          console.log('Render: [timer] seek-based loop, totalFrames:', totalFrames);
          (async () => {
            try {
