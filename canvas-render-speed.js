(function () {
  ...
  // Does the 2D context accept an SVG url() filter? (Chromium yes; needed for the grade's
  // sharpen pass — colour grade lands regardless via _supportsCtxFilter.) We test that the
  // string is accepted; where it isn't, render skips sharpen and the colour grade still applies.
  const _supportsCtxFilterUrl = (() => {
    try {
      const testCtx = document.createElement('canvas').getContext('2d');
      if (!testCtx || typeof testCtx.filter !== 'string') return false;
      const prev = testCtx.filter;
      testCtx.filter = 'url(#clipSharpen)';
      const ok = testCtx.filter === 'url(#clipSharpen)' || testCtx.filter.includes('clipSharpen');
      testCtx.filter = prev;
      return ok;
    } catch (_) {
      return false;
...
if (kickLogo && barEl) {
  ...
  if (bR) {
    ...
    function _effCrop() {
      ...
      const c = cid && _clipById.get(cid);
      // TIME-VARYING crop (facecamTrack keyframes, absolute source secs) — follows a moving person.
      // Mirrors the preview's per-frame item._crop update; interpolator shared via window._fcTrackAt.
      const fct = c && c.override && c.override.facecamTrack;
      if (fct && fct.length && window._fcTrackAt && _eng.currentOutputTime) {
        const _spd = window.canvasAPI?.clipSpeed?.(c) ?? 1;
        const srcSec = c.sourceStart + (_eng.currentOutputTime() - c.outputStart) * _spd;
        const b = window._fcTrackAt(fct, srcSec);
        if (b && isFinite(b.x1)) return b;
      }
    ...
    return new Promise((resolve, reject) => {
      ...
      const _onSeeked = async function onSeeked() {
        ...
        function drawCanvasFrame(t) {
          ...
          // Video-layer GRADE string for this frame — same numbers as the live .is-graded CSS rule
          // (getGradeFilter → '' when the toggle is off). Colour needs _supportsCtxFilter; the url()
          // sharpen is appended only where the canvas accepts it AND the SVG is present. Applied to
          // the VIDEO picture only (below, before overlays) + the floating chat — never gifs/text/badges.
          const _gF   = _supportsCtxFilter ? (api.getGradeFilter?.(false) || '') : '';
          const _grade = (_gF && _supportsCtxFilterUrl && document.getElementById('clipSharpen'))
            ? (api.getGradeFilter(true) || _gF) : _gF;
          ...
          // GRADE the video picture (backdrop + gameplay + facecam + chat-split) — redraw everything
          // painted SO FAR through ctx.filter, BEFORE any overlay draws. Gifs, text, watermark and the
          // badge all draw after this → they stay ungraded, matching the live video-hold-scoped CSS.
          if (_grade) {
            ctx.save();
            ctx.filter = _grade;
          ...
          if (!_chatInSplit && chatVid && chatLayout && state.chatVisible) {
            ...
            if (_grade) ctx.filter = _grade;   // grade the chat video too (matches live .chat_hold); reset by restore()
...

          {
            ...
            if (titleLines.length && state.titleVisible) {
              ...
              const _tot = _useEngine ? _eng.currentOutputTime() : t;   // OUTPUT time — title window is output-anchored (follows reorder)
              if (_tot >= titleStart && _tot <= titleEnd) renderTitleAtY(ctx, titleLines, titleY, ts);
            }
