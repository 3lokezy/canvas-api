      ...
      window.canvasAPI = {
        ...
        setGameplayZoom(z) {
          ...
          if (mode === 'is-full' || mode === 'is-overlay') {
            ...
            modeStates[mode] = { zoom: getGpZoom(), panX: gpLeft / gpHW, panY: gpTop / gpHH };
          }
        },
    ...
    function initCanvas(item, opts = {}) {
      ...
      function restoreModeZoomPan(mode) {
        ...
        const doRestore = () => {
          ...
          gpVideo.style.width  = `${dw}px`;
          gpVideo.style.height = `${dh}px`;
          gpVideo.style.left   = `${Math.max(hw - dw, Math.min(0, saved.panX * hw))}px`;
          gpVideo.style.top    = `${Math.max(hh - dh, Math.min(0, saved.panY * hh))}px`;
        };
        ...
        makeDraggable(gpHold, {
          ...
          onEnd: () => {
            ...
            if (mode === 'is-full' || mode === 'is-overlay') {
              ...
              const gpHW   = gpHold.clientWidth  || 1;
              const gpHH   = gpHold.clientHeight || 1;
              modeStates[mode] = { zoom: gpZoom, panX: gpLeft / gpHW, panY: gpTop / gpHH };
            }
          },
        });
      ...
      function applyModeFn(mode) {
        ...
        // Save current mode's zoom/pan before leaving
        if (leavingMode && leavingMode !== mode &&
            (leavingMode === 'is-full' || leavingMode === 'is-overlay')) {
          const gpLeft = parseFloat(gpVideo.style.left) || 0;
          const gpTop  = parseFloat(gpVideo.style.top)  || 0;
          const gpHW   = gpHold.clientWidth  || 1;
          const gpHH   = gpHold.clientHeight || 1;
          modeStates[leavingMode] = { zoom: gpZoom, panX: gpLeft / gpHW, panY: gpTop / gpHH };
        }
        ...
        // After applyMode's split RAF, restore the entering mode's saved state
        // or apply correct default zoom if no saved state
        requestAnimationFrame(() => {
          requestAnimationFrame(() => {
            const enteringMode = frame._currentMode;
            if (enteringMode === 'is-split') return; // handled by applyMode RAF
    
            const restored = restoreModeZoomPan(enteringMode);
            if (!restored && gpVideo.videoWidth) {
              // No saved state and video ready — apply correct default
              const defaultZoom = calcDefaultZoom(gpHold, gpVideo);
              if (defaultZoom !== null) applyGpZoom(defaultZoom);
            }
            // If video not ready and no saved state, loadedmetadata handles it
          });
        });
      }
