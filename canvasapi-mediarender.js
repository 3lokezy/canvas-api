(function () {
    ...
    function buildPanelAPI(refs) {
      ...
      window.canvasAPI = {
        ...
        // ── Render — client-side export to WebM ─────────────────────────────────
        async render(onProgress) {
          if (frame._rendering) throw new Error('Render already in progress');
          if (!gpVideo?.videoWidth) throw new Error('Video not loaded');
          ...
          const wasPaused = gpVideo.paused;
          const wasTime   = gpVideo.currentTime;
          const wasVol    = gpVideo.volume;
          gpVideo.pause();
          ...
          try {
            const vs = gpVideo.captureStream();
            vs.getAudioTracks().forEach(t => stream.addTrack(t));
          } catch (_) {
            console.warn('Render: audio capture unavailable (CORS), exporting video-only');
          }
          ...
          const gpHoldH  = mode === 'is-split' ? OUT_H * (getSplitPct() ?? 0.5) : OUT_H;

          const vw = gpVideo.videoWidth, vh = gpVideo.videoHeight;
          const coverScale = Math.max(OUT_W / vw, gpHoldH / vh);
          const zoom = getGpZoom();
          ...
          const renderL = dw > OUT_W
            ? Math.max(OUT_W  - dw, Math.min(0, panCX * OUT_W  - dw / 2))
            : (OUT_W - dw) / 2;
          const renderT = dh > gpHoldH
          ...
          return new Promise((resolve, reject) => {
            recorder.onstop = () => {
              ...
              gpVideo.volume = wasVol;
              if (!wasPaused) {
                gpVideo.currentTime = wasTime;
              }
            ...
            function startRender() {
              recorder.start(100);
              gpVideo.play().catch(reject);

              function draw() {
                if (!frame._rendering) return;

                const t = gpVideo.currentTime;
                if (t >= effectiveEnd - 0.03 || gpVideo.ended) {
                  gpVideo.pause();
                ...
                ctx.clip();
                ctx.drawImage(gpVideo, 0, 0, vw, vh, renderL, renderT, dw, dh);
                ctx.restore();
            ...
            gpVideo.currentTime = effectiveStart;
            gpVideo.addEventListener('seeked', startRender, { once: true });
          });
        },
      };
      ...
      // ── Render button binding ──────────────────────────────────────────────
      const renderBtn = document.getElementById('render_button')
        ?? document.querySelector('[wized="render_button"]');

      window.renderClip = async function () {
        if (!window.canvasAPI) return;
        const btn = renderBtn;
        const origText = btn?.textContent;
        if (btn) { btn.textContent = 'Rendering… 0%'; btn.style.pointerEvents = 'none'; btn.style.opacity = '0.6'; }

        try {
          const blob = await window.canvasAPI.render(progress => {
          ...
          const clip = document.querySelector('[wized="stream_clip_id"]')?.textContent.trim() || 'clip';
          a.href     = url;
          a.download = `${clip}-render.webm`;
      ...
      if (renderBtn) {
        renderBtn.addEventListener('click', () => window.renderClip());
      }
    }
