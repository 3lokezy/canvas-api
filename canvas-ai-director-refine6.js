(function () {
  ...
  const CFG = (window.aiDirectorCfg = Object.assign({
    ...
    // SCREEN-CORRELATION ticker (cam clips): a split only earns its top panel when the SCREEN is doing something.
    SCREEN_LOW_REL: 0.6,    // beat screen-motion below this × the clip's OWN median = static → fill with the cam
    SCREEN_LOW_ABS: 0.005,  // absolute floor so an all-static clip reads static (not "half below its own median")
    CAM_FILL_ZOOM: 1.5,     // fullscreen-cam zoom when a static-screen resting beat fills the frame with the streamer
  ...
  const FEATURES = {
    ...
    titles(plan, a) {
      ...
      if (Number.isFinite(+plan.title_out) && +plan.title_out > 0) {
        ...
        a.setTitleTiming?.(inOt, outOt);   // OUTPUT seconds — the title gate is output-time now (follows reorder)
    ...
    // places them. SOURCE-anchored (at=from, dur=to−from, both source seconds) — getImageCues resolves to output
    // live, so gifs follow clip reorder/delete. Empty/absent ⇒ leave existing images (a run that adds none doesn't wipe manual ones).
  ...
  function frameFull(absFrom, absTo, zoom, fx, fy) {
    ...
    // DELIBERATE CAM PUNCH — the caller aimed at an edge/corner target (a facecam) with real zoom.
    // Two failure modes lived here: ① a central content face hijacked the focus via the snap below;
    // ② for a cam near the top/bottom of the source the frame-edge clamp parks the face at the very
    // border of the comp (vertical span = 1/zoom of source height, so at low zoom a bottom face CANNOT
    // be centred). Detect the aim, skip the snap, and zoom until the face sits near eye level.
    const tEdge = Math.min(fx, 1 - fx, fy, 1 - fy);
    if (zoom > 1.15 && tEdge < CFG.EDGE_NEAR) {
      const face = sc ? (sc.faces || []).find(f => Math.abs(f.cx - fx) < 0.08 && Math.abs(f.cy - fy) < 0.08) : null;
      const fsz = Math.max(0.025, (face && face.size) || 0.05);
      const needV = fy > 0.5 ? 0.45 / Math.max(0.05, 1 - fy)          // bottom cam: face up to ~0.45 frame height
                  : fy < 0.35 ? 0.35 / Math.max(0.05, fy) : 1;        // top cam: down to ~0.35
      const faceCap = CFG.LB_FIT * 0.55 / fsz;                        // face ≤ ~55% of the visible width
      zoom = +Math.max(zoom, Math.min(Math.max(needV, 1.6), faceCap, 3.4)).toFixed(2);
    } else if (sc && sc.primary) {
    ...
    // Clamp so the focus target stays inside the frame — PER AXIS. Vertically the cover view spans 1/zoom
    // of source height, but horizontally (16:9 source in a 9:16 frame) it spans only ~LB_FIT/zoom of source
    // width. Clamping X with 0.5/zoom drags corner subjects (a facecam at cx≈0.18) OUT of the visible window.
    const my = 0.5 / zoom;
    const mx = Math.min(0.5, CFG.LB_FIT / (2 * zoom) + 0.02);
    return { zoom, focusX: Math.max(mx, Math.min(1 - mx, fx)), focusY: Math.max(my, Math.min(1 - my, fy)) };
  }
  ...
  // The dominant graph identity within an ABSOLUTE source range (most face rows). Graph-only — matches
  // buildTrack's own pid pick, and returns null without a scene graph (scan-only clips skip the relay).
  function _dominantPersonIn(absFrom, absTo) {
    if (!sceneGraph()) return null;
    const c = {};
    for (const r of sgRows(absFrom, absTo)) for (const f of r.faces) c[f.p] = (c[f.p] || 0) + 1;
    return Object.keys(c).sort((x, y) => c[y] - c[x])[0] || null;
  }
  // The cleanest capture moment for an identity across its tracked clips: the graph row where its face
  // is BIGGEST. Returns { sx, sy, atSec (absolute source) } for captureFaceAt, or null.
  function _bestFaceMoment(pid, clips) {
    if (!sceneGraph()) return null;
    const o = sgOff();
    let best = null, bestW = 0;
    for (const c of clips) {
      for (const r of sgRows(c.sourceStart, c.sourceEnd)) {
        for (const f of r.faces) {
          if (f.p !== pid) continue;
          const w = f.b[2] - f.b[0];
          if (w > bestW) { bestW = w; best = { sx: (f.b[0] + f.b[2]) / 2, sy: (f.b[1] + f.b[3]) / 2, atSec: r.t + o }; }
        }
      }
    }
    return best;
  }
  // TRACKED-FACE RELAY — after an AI edit lands, export a face image for each identity the edit actually
  // follows and surface it in the character library (the "face images we have in the tracking UI"), then
  // tag each tracked clip so selecting it lights the right slot. Reuses the prebuilt capture→character
  // pipeline: captureFaceAt (click-free, off-engine decode) → addCharacter → setClipsCharacters. Runs in
  // the BACKGROUND (fire-and-forget), graph-only, dedup by identity, and never throws into the apply.
  let _faceRelayBusy = false;
  async function _relayTrackedFaces() {
    if (_faceRelayBusy) return;
    _faceRelayBusy = true;
    try {
      const a = api();
      const g = sceneGraph();
      if (!a || !g || !window.wcTracker?.captureFaceAt || !a.addCharacter) return;
      const tracked = (a.getOutputClips?.() || []).filter(c =>
        c.override?.tracking?.enabled && c.override.tracking.raw?.length && c.override.tracking.person);
      if (!tracked.length) return;
      // group the tracked clips by identity
      const byPid = new Map();
      for (const c of tracked) {
        const pid = c.override.tracking.person;
        if (!byPid.has(pid)) byPid.set(pid, []);
        byPid.get(pid).push(c);
      }
      const speakerOf = {}; for (const p of (g.persons || [])) speakerOf[p.id] = p.speaker;
      const existing = new Map((a.getCharacters?.() || []).map(ch => [ch.name, ch.id]));   // dedup by name = pid/speaker
      const tag = {};   // clipId → charId
      for (const [pid, clips] of byPid) {
        const label = speakerOf[pid] || pid;
        let charId = existing.get(label) || existing.get(pid);
        if (!charId) {
          const moment = _bestFaceMoment(pid, clips);
          if (!moment) continue;
          const cap = await window.wcTracker.captureFaceAt(moment);
          if (!cap || !cap.emb) { console.log('[ai-polish] face relay: no face captured for ' + pid); continue; }
          charId = a.addCharacter({ emb: cap.emb, thumb: cap.thumb, sx: cap.sx, sy: cap.sy, atSec: cap.atSec, name: label });
          if (!charId) continue;
          existing.set(label, charId);
          console.log('[ai-polish] face relay: added ' + label + ' (' + clips.length + ' clip' + (clips.length === 1 ? '' : 's') + ')');
        }
        for (const c of clips) tag[c.id] = charId;
      }
      if (Object.keys(tag).length && a.setClipsCharacters) a.setClipsCharacters(tag);
    } catch (e) {
      console.warn('[ai-polish] face relay failed', e);
    } finally {
      _faceRelayBusy = false;
    }
  }
  ...
  function isRealOverlayCam() {
    // Trust the SIDECAR's overlay-cam classification over anything measured client-side. The client's own
    // BlazeFace scan has the same small-corner-cam blindness we fixed on the VPS (a 5%-width cam is
    // invisible to a ~192px detector), so a react/gaming clip with a tiny cam scanned as "no cam" here and
    // compiled full/full/full — even though the n8n digest plainly says OVERLAY_CAM. Authoritative order:
    //   ① response facecam.type  ② the digest scan type (built from the sidecar, survives blind BlazeFace)
    //   ③ graph  ④ wized flag / client scan.
    const T = new Set(['OVERLAY_CAM', 'DUO_OVERLAY']);
    // SHOT-TYPE GATE: if the clip's DOMINANT shots are a conversation/IRL scene, a corner "facecam" is an
    // incidental background face (a wall picture, a third person, a TV) — NOT a streamer overlay. Suppress
    // it so IRL clips don't inherit cam-clip assumptions (the false-positive top-right cam on CONVO_IRL).
    const g0 = sceneGraph();
    if (g0 && Array.isArray(g0.shots) && g0.shots.length) {
      let cam = 0, convo = 0;
      for (const s of g0.shots) {
        const d = (s.to - s.from) || 0;
        if (s.type === 'CAM_OVERLAY' || s.type === 'OVERLAY_CAM' || s.type === 'DUO_OVERLAY') cam += d;
        else if (/IRL|TALKING_HEAD/.test(s.type || '')) convo += d;
      }
      if (convo > cam) return false;
    }
    if (_planFacecam && T.has(_planFacecam.type)) return true;
    const scan = window.__aiDigest && window.__aiDigest.scan;
    if (Array.isArray(scan) && scan.some(s => T.has(s.type))) return true;
    const g = sceneGraph();                              // POSITIVE-only: a stale graph's found:false must
    if (g && g.facecam && g.facecam.found && T.has(g.facecam.type)) return true;   // never VETO the signals above
    if (_planFacecam?.box && !_planFacecam.type) return true;   // a box with no type at all — trust it (split beats full/full/full)
  ...
  function applyPlan(plan) {
    ...
    const clips = v.segments.map(s => {
      ...
      if (s.track) {                                                    // scan-derived follow track for this clip
        ...
        if (raw && raw.length >= 2) { c.override.tracking = { enabled: true, raw, mode: 'face', person: (s.person || _dominantPersonIn(off + s.from, off + s.to)) || undefined }; console.log('[ai-polish] track', s.from.toFixed(1) + '–' + s.to.toFixed(1) + 's', raw.length + 'pts'); }
        ...
        if (fs.length >= 4) {
          ...
          if (spread > CFG.MOVE_THRESH) {
            ...
            if (raw && raw.length >= 2) {
              c.override.tracking = { enabled: true, raw, mode: 'face', person: s.person };
    ...
    setTimeout(() => { _relayTrackedFaces(); }, 0);   // background: export the tracked faces → character library (non-blocking)
    setTimeout(() => { a.fitTimelineZoom?.(); }, 80);  // long source clipped down → auto-zoom the timeline to the kept length
    return v;
  }
  function revert() {
    const a = api(); if (!a) return;
  ...
  function generatePlan(opts = {}) {
    // DEGRADE GRACEFULLY: the conversation grammar wants a graph + diarization, but a GAMING clip needs
    // neither — the cam context runs off baseCamBox() (row/DOM facecam), and undiarized words still give
    // word-gap snapping + pseudo-turns (split at long silences). Old clips must compile, not bail.
    const realG = sceneGraph();
    const g = realG || { persons: [], speakers: {}, timeline: [], shots: [] };
    if (!realG) console.warn('[ai-polish] auto: no scene graph — cam/scan fallback grammar');
    const off = api()?.getSourceWindow?.()?.start || 0;
    const srcDur = sourceDuration() || 0;
    // SCREEN-CORRELATION ticker: mean scan screen-motion (gameplay region, cam/faces excluded) over an ABSOLUTE
    // window, and a per-clip "static" floor calibrated to THIS clip's own median (same trick as the gap bands, so
    // a chill IRL stream and a frantic FPS clip get different baselines). screenMotionIn returns null when the
    // scan didn't cover the window (or lacks the field on a stale scan) → the caller falls back to the split weave.
    const _pfS  = window._scan?.perFrame || [];
    const _sVals = _pfS.map(f => f.screen).filter(v => typeof v === 'number' && v > 0).sort((x, y) => x - y);
    const _sMed  = _sVals.length ? _sVals[Math.floor(_sVals.length / 2)] : 0;
    const _sFloor = Math.max(_sMed * CFG.SCREEN_LOW_REL, CFG.SCREEN_LOW_ABS);
    const screenMotionIn = (a, b) => {
      let s = 0, n = 0;
      for (const f of _pfS) { if (f.t < off + a || f.t > off + b) continue; if (typeof f.screen === 'number') { s += f.screen; n++; } }
      return n ? s / n : null;
    };
    const words = readTranscript();
    if (!words.length) { console.warn('[ai-polish] auto: needs a transcript'); return { segments: [] }; }
    if (!words[0].speaker) console.warn('[ai-polish] auto: transcript not diarized — single-voice turns (gap-split)');
    ...
    // 3. beats
    // The clip's fixed overlay cam is LOOP-INVARIANT — compute it once (was re-derived every turn via
    // isRealOverlayCam()+baseCamBox()). `inCam` takes a measured position.
    const camB = isRealOverlayCam() ? baseCamBox() : null;
    const inCam = pos => pos && camB && pos.cx > camB.x1 && pos.cx < camB.x2 && pos.cy > camB.y1 && pos.cy < camB.y2;
    const segs = [];
    for (const t of work) {
      let feat = spk2p[t.sp] || null;
      if (!feat) {
        // UNBOUND speaker (diarization heard a voice with no hard face-bind — common on multi-person IRL:
        // 4 diarized voices, only 1 face bound). Infer the featured person from measured mouth-motion: the
        // most face-active NON-cam person in the turn. Turns a 2-person conversation from flat letterbox
        // into proper duo splits. On a gaming clip the cam is excluded here, so the turn stays ctx='cam'.
        let bestMm = 0.04, bestId = null;
        for (const p of g.persons) {
          const pos = personPosIn(p.id, off + t.from, off + t.to);
          if (!pos || inCam(pos)) continue;
          const m = mmIn(p.id, t.from, t.to);
          if (m > bestMm) { bestMm = m; bestId = p.id; }
        }
        if (bestId) feat = bestId;
      }
      const featPos = feat && personPosIn(feat, off + t.from, off + t.to);
      // "OTHER PARTICIPANT" = a real SECOND person in a conversation — NEVER the corner cam. When a speaker is
      // unbound (feat=null: diarization over-split one streamer into two voices, or a teammate on comms with no
      // face), the old filter (p.id!==feat) captured the cam person AS the other → blocked ctx='cam' → forced
      // fullscreen/letterbox (the "gameplay clip lands mostly full" bug). Excluding inCam persons keeps unbound
      // cam-clip turns on the split grammar. Positions are computed ONCE here and reused (was 2× per person).
      const others = g.persons
        .map(p => ({ id: p.id, presence: p.presence, pos: p.id !== feat ? personPosIn(p.id, off + t.from, off + t.to) : null }))
        .filter(o => o.pos && !inCam(o.pos))
        .sort((a, b) => (b.presence || 0) - (a.presence || 0));
      const other = others[0] ? others[0].id : null;
      const otherPos = others[0] ? others[0].pos : null;
      ...
      console.log('[ai-polish] turn ' + t.from.toFixed(1) + '–' + t.to.toFixed(1) + 's ' + t.sp + '→' + (feat || '?') +
                  ...
                  ' beats=' + (bounds.length - 1) + (t.feature ? ' intent=' + t.feature : ''));
      // A reaction intent on a LONG range lands on its KEY beat (the reactor's mouth-motion peak), not on
      // every beat — 12s of wall-to-wall reaction closeup buries the context the letterbox weave provides.
      let featBeat = 0;
      if (t.feature === 'reaction' && bounds.length - 1 > 2) {
        const who = t.fperson || reactOn;
        let best = -1;
        for (let i = 0; i < bounds.length - 1; i++) {
          const m = who ? mmIn(who, bounds[i], bounds[i + 1]) : 0;
          if (m > best) { best = m; featBeat = i; }
        }
      }
      for (let i = 0; i < bounds.length - 1; i++) {
        const a = bounds[i], b = bounds[i + 1];
        const esc = CFG.TRACK_PUNCH + i * 0.15;
        // GAMING BIAS: on a cam clip the split (gameplay + dedicated reaction cam) IS the format — the
        // full-wide accent comes rarer (every 4th beat, not 3rd), and emphasis intents stay INSIDE the split.
        const cad = ctx === 'cam' ? CFG.SOLO_EVERY + 1 : CFG.SOLO_EVERY;
        const solo = i > 0 && i % cad === cad - 1;
        let s = null;
        ...
        // reaction/punch are EMPHASIS intents: on ranges longer than 2 beats they claim only their key
        // beat(s) — reaction at the mm peak, punch on the first two — and the rest rides the default
        // grammar (which keeps both faces on screen). wide/letterbox/split hold for the whole range.
        let feature = t.feature || null;
        if (bounds.length - 1 > 2) {
          if (feature === 'reaction' && i !== featBeat) feature = null;
          else if (feature === 'punch' && i > 1) feature = null;
          // on a cam clip 'wide' is an ACCENT too — establish for two beats, then back to the split;
          // a 10s wide range otherwise holds full-gameplay and the facecam vanishes for the whole stretch
          else if (feature === 'wide' && ctx === 'cam' && i > 1) feature = null;
        }
        if (feature === 'wide') {
          // RAIL WINS FORMAT (Q2: cam always present): on a cam clip "show the action" = FULL-WIDTH gameplay
          // on top with the streamer still in the bottom panel — never a full frame that deletes the cam.
          if (ctx === 'cam') s = { from: a, to: b, mode: 'is-split', zoom: 1, bottom_zoom: +Math.min(1.4, 1 + i * 0.06).toFixed(2) };
          else s = { from: a, to: b, mode: 'is-full', zoom: 1 };
        } else if (feature === 'letterbox') {
          // a letterbox "the whole scene matters" on a cam clip ALSO keeps the cam: content wide on top, cam bottom.
          if (ctx === 'cam') s = { from: a, to: b, mode: 'is-split', zoom: 1, bottom_zoom: +Math.min(1.4, 1 + i * 0.06).toFixed(2) };
          else s = letterboxSeg(a, b, featPos, otherPos);
        } else if (feature === 'punch' && (t.fperson || feat || camP)) {
          // GAMING: emphasis stays INSIDE the split — punch INTO the cam panel, gameplay keeps playing.
          // Fullscreen cam is reserved for the measured reaction peak (the auto cutaway).
          if (ctx === 'cam') s = { from: a, to: b, mode: 'is-split', zoom: 1.05,
                                   bottom_zoom: +Math.min(1.6, 1.3 + i * 0.1).toFixed(2) };
          else s = { from: a, to: b, mode: 'is-full', person: t.fperson || feat, zoom: +Math.min(CFG.ZOOM_MAX, esc).toFixed(2) };
        } else if (feature === 'reaction' && (t.fperson || reactOn)) {
          // cam ctx: a reaction IS the face — fullscreen eye-level cam on the key beat (a 'Streamer
          // Reacts' clip must actually show the streamer), split with a tight cam panel around it
          if (ctx === 'cam') s = i === featBeat
              ? { from: a, to: b, mode: 'is-full', person: camP || reactOn, zoom: 1.6, react: true }
              : { from: a, to: b, mode: 'is-split', zoom: 1, bottom_zoom: 1.5, react: true };
          else s = { from: a, to: b, mode: 'is-full', person: t.fperson || reactOn,
                     zoom: +Math.min(CFG.ZOOM_MAX, CFG.TRACK_PUNCH + 0.2).toFixed(2), react: true };
        } else if (feature === 'split') {
          if (ctx === 'cam') s = { from: a, to: b, mode: 'is-split', zoom: +Math.min(1.3, 1 + i * 0.06).toFixed(2),
                                   bottom_zoom: +Math.min(1.4, 1 + i * 0.08).toFixed(2) };
        ...
        if (i === reactIdx && reactOn) {
          // cam cutaway zoom stays LOOSE: the cover view is only ~LB_FIT of source width, so z2.0 shows
          // ~16% of the frame — barely wider than the face itself. ~1.25 puts the cam face at half-frame.
          s = { from: a, to: b, mode: 'is-full', person: reactOn,
                zoom: +Math.min(CFG.ZOOM_MAX, ctx === 'cam' ? 1.25 : CFG.TRACK_PUNCH + 0.2).toFixed(2), react: true };
        } else if (ctx === 'cam') {
          // split is the resting state; the accent beat ALTERNATES full-wide gameplay ↔ fullscreen cam
          // punch (eye-level framed by frameFull) so long cam clips aren't split-only with one rare
          // cam moment. The cam panel tightens through the turn so consecutive splits read differently.
          const camShot = camP && Math.floor(i / cad) % 2 === 1;
          const weave = solo ? (camShot ? { from: a, to: b, mode: 'is-full', person: camP, zoom: 1.6 }
                                        : { from: a, to: b, mode: 'is-full', zoom: 1 })
                             : { from: a, to: b, mode: 'is-split', zoom: +Math.min(1.3, 1 + i * 0.06).toFixed(2),
                                 bottom_zoom: +Math.min(1.4, 1 + i * 0.08).toFixed(2) };
          // SCREEN-CORRELATION ticker: a split (or full-wide gameplay) only earns the top panel when the SCREEN is
          // doing something. Static screen this beat + a cam to fill with → drop the split and fill the frame with
          // the streamer (the "gameplay isn't on / talking to camera" case). Live screen → the normal weave. The
          // reaction/wide/punch intents above + overlap-refusal are untouched — this only rewrites the DEFAULT beat.
          const sa = screenMotionIn(a, b);
          if (camP && sa != null && sa < _sFloor) {
            s = { from: a, to: b, mode: 'is-full', person: camP, zoom: solo ? 1.6 : CFG.CAM_FILL_ZOOM };
            console.log('[ai-polish] cam beat ' + a.toFixed(1) + '–' + b.toFixed(1) + 's screen=' + sa.toFixed(4) + ' < floor ' + _sFloor.toFixed(4) + ' → CAM-FILL');
          } else {
            s = weave;
            if (sa != null) console.log('[ai-polish] cam beat ' + a.toFixed(1) + '–' + b.toFixed(1) + 's screen=' + sa.toFixed(4) + ' ≥ floor ' + _sFloor.toFixed(4) + ' → ' + weave.mode);
          }
        } else if (ctx === 'duo') {
    ...
    for (let i = 1; i < segs.length; i++) {
      const sig = s => s.mode + '|' + (s.person || '') + '|' +
                       Math.round((((s.mode === 'is-letterbox' ? s.lzoom : s.zoom) || 1) + (s.bottom_zoom || 0)) * 5);
  ...
  function applyIntents(plan) {
    ...
    // Set the authoritative facecam NOW, before the grammar runs — generatePlan() below calls
    // isRealOverlayCam()/baseCamBox(), and those key on _planFacecam. Previously it was only set inside
    // applyPlan() (which runs AFTER generatePlan), so the react/cam grammar saw a null cam and compiled
    // every intent to full. THIS is why a clip with a valid facecam still landed full/full/full.
    if (p.facecam && p.facecam.box) _planFacecam = p.facecam;
    ...
    // ① holes shorter than 1.5s NEVER survive (a skip that short reads as a glitch, not an edit — the
    // wall-kiss shredding): same-intent neighbours MERGE into one scene; different-intent neighbours keep
    // both intents but the earlier range extends to swallow the hole.
    ...
    for (const r of keep) {
      const last = merged[merged.length - 1];
      if (last && r.from - last.to <= HOLE && r.from >= last.from &&    // forward neighbour/overlap, not a reorder
          ((r.feature || null) === (last.feature || null) && (r.person || null) === (last.person || null)
           || (r.to - r.from) < CFG.MIN_SEG)) last.to = Math.max(last.to, r.to);
      else {
        if (last && r.from - last.to > 0 && r.from - last.to <= HOLE) last.to = r.from;   // bridge the micro-hole
        merged.push({ ...r });
      }
    }
    for (let i = 0; i < merged.length; i++) {
      ...
      if (prev && r.from - prev.to <= HOLE) { prev.to = r.to; merged.splice(i--, 1); }
      else if (next && next.from - r.to <= HOLE) { next.from = r.from; merged.splice(i--, 1); }
    }
    ...
    if (p.facecam && p.facecam.box) det.facecam = p.facecam;          // carry the cam into applyPlan (else its line ~1092 re-sets _planFacecam=null and the split panels lose the crop)
    ...
    // TIME: the model speaks SOURCE time for everything (segments + sfx/image cue times) plus an optional
    // storyboard `anchor` (cell id). We only turn an anchor into its SOURCE second here; the cues are stored
    // SOURCE-anchored (SOUNDBOARD_ANCHOR / IMAGE_ANCHOR = 'source' in canvas-api) and getSoundboardCues /
    // getImageCues resolve source→output LIVE against the current clips. So sfx/gifs land on their beat AND
    // then FOLLOW any later clip reorder/delete/trim automatically — no re-run, and cut-away cues just drop.
    try {
      ...
      for (const c of (det.sfx || [])) {
        const s = anchorSrc(c);
        if (s != null) { c.at = +s.toFixed(2); console.log('[ai-polish] sfx anchor ' + c.anchor + ' → source ' + s.toFixed(1) + 's'); }
      }
      for (const c of (det.images || [])) {
        const s = anchorSrc(c);
        if (s != null) { const d = Math.max(0.5, (+c.to - +c.from) || 2); c.from = +s.toFixed(2); c.to = +(s + d).toFixed(2); console.log('[ai-polish] image anchor ' + c.anchor + ' → source ' + s.toFixed(1) + 's'); }
      }
    } catch (_) {}
  ...
  // ROUTE ANY PLAN to the right applier — stale button handlers can't pick wrong. A v3 INTENTS plan has
  // no per-segment `mode` (layout is the rail's job); a legacy geometry plan always does. Also swallows
  // duplicate applies (double-bound handlers would snapshot the already-edited state and break revert).
  let _lastSig = null, _lastAt = 0;
  function applyAny(plan) {
    const p = coercePlan(plan);
    let sig = ''; try { sig = JSON.stringify(p && p.segments); } catch (_) {}
    if (sig && sig === _lastSig && Date.now() - _lastAt < 10000) {
      console.warn('[ai-polish] duplicate apply ignored (same plan twice within 10s — double-bound handler?)');
      return;
    }
    _lastSig = sig; _lastAt = Date.now();
    const intents = p && Array.isArray(p.segments) && p.segments.length && p.segments.every(s => s && s.mode == null);
    if (intents) { stage('④', 'plan has no per-segment modes → v3 INTENTS plan, routing through the grammar'); return applyIntents(p); }
    stage('④', 'plan carries per-segment modes → legacy geometry plan, applying directly');
    return applyPlan(p);
  }
  ...
  async function run() { const d = await buildDigest(); if (!d) return; return applyAny(await fetchPlan(d)); }
  ...
  window.aiPolish.apply    = applyAny;   // webhook response (parsed plan OR raw envelope) — auto-routes intents vs legacy
  window.aiPolish.dryRun   = applyAny;   // hand-pasted plan to test — same auto-routing
