(function () {
  ...
  const _RENDER_SCRIPT_BUILD = 'pb-20260502e-finalize-deadlock';
    ...
    return new Promise((resolve, reject) => {
      ...
      async function finishMp4() {
        try {
          ...
          // Do NOT await mp4AudioAddPromise before finalize — deadlock.
          // AudioBufferSource.add() only resolves after the encoder output is
          // flushed, which happens during output.finalize(). Awaiting it here
          // means finalize never runs and the Promise never settles.
          // audioSource.close() queues the encoder flush; finalize drains it.
          videoSource.close();
