(function () {
    ...
    document.addEventListener(
      ...
      (e) => {
        ...
        switch (e.key) {
          case ' ':
            e.preventDefault();
            gpVideo.paused ? api.play() : api.pause();
            break;
          case 'ArrowLeft':
            e.preventDefault();
            api.seekTo(Math.max(getWindowOffset(), gpVideo.currentTime - NUDGE_FRAMES));
            break;
          case 'ArrowRight':
            e.preventDefault();
            api.seekTo(Math.min(getWindowOffset() + getDisplayDuration(), gpVideo.currentTime + NUDGE_FRAMES));
            break;
          case '[': {
            e.preventDefault();
            const t = api.getTracks().video;
  ...
  function mountNLETrack(masterRow, api, getDisplayDuration) {
    ...
    // The NLE timeline spans OUTPUT duration (sum of clip lengths), not absolute
    // source end — otherwise a windowed (vod) clip whose source ends at e.g. 2040s
    // scales the timeline to 2040s and the real window becomes a sliver.
    const masterDuration = initialClips.length
    ...
    function bindBar(el, barObj) {
      ...
      el.addEventListener('pointerdown', (e) => {
        ...
        if (_toolMode === 'delete') {
          if (barObj.clipId && api.deleteClip) { api.deleteClip(barObj.clipId); masterRow._nleRebuild?.(); }
          return;
        }
      ...
      if (hL) hL.addEventListener('pointerdown', (e) => {
        ...
        _nleSeek(e.clientX);
        startAction(e, barObj, 'resize-left');
      });
    ...
    function updateTitleTags() {
      bars.forEach((b, i) => {
        const tag = b.element.querySelector('.title_tag');
    ...
    function handleMove(e) {
      ...
      if (!_actionDidMove) {
        _actionDidMove = true;
        if (activeAction === 'drag') targetBar.element.classList.add('is-dragging');
      }
      ...
      if (activeAction === 'drag') {
        ...
        if (rollNeighbour) {
          ...
          let newW = Math.max(MIN_W, initialBarWidth + deltaX);
        ...
        if (rollNeighbour) {
          ...
          let newX = initialBarX + deltaX;
          let newW = initialBarWidth - deltaX;
          let minX = gap;
    ...
    function selectClip(bar) {
      bars.forEach(b => b.element.classList.toggle('is-selected', b === bar));
    }
    ...
    function syncClipsFromBars() {
      ...
      const newClips = bars.map(b => {
        ...
        return {
          id:          b.clipId,
          sourceStart: b.sourceIn  * masterDuration,
          sourceEnd:   b.sourceOut * masterDuration,
          outputStart: (b.currentX / trackW) * masterDuration,
        };
      });
    ...
    masterRow._nleSplitBar = (xInRow) => {
      const curW = masterRow.clientWidth;
      if (curW > 0 && bars.length === 1 && bars[0].currentX === 0) {
        bars[0].width = curW;
        bars[0].element.style.width = `${curW}px`;
      }
  ...
  function initTimeline() {
    ...
    function updateTimeLabels() {
      ...
      if (isNLE) {
        ...
        if (elEnd)   elEnd.textContent   = fmt(api.getOutputDuration?.() ?? 0);
      } else {
