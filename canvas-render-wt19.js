  ...
  if (bR) {
    ...
    if (useMp4) {
      ...
      setupMp4AudioSource = async () => {
        ...
        // For in-source clips on desktop: use raw AAC extraction instead of
        // full-file PCM decode. Avoids loading the full VOD into RAM.
        if (isInSource && typeof AudioEncoder !== 'undefined' && !_isIOS && !_rawAudioPromise) {
          ...
          _rawAudioPromise = extractRawMp4Audio(
            ...
            rawAudioData = data;
            console.log('Render: in-source raw AAC extracted —',
              rawAudioData ? rawAudioData.samples.length + ' samples' : 'no audio track found');
          }).catch(e => {
            console.warn('Render: in-source audio extraction failed (video-only fallback):', e);
          });
          return;
        }
