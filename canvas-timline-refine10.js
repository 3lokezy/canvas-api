(function () {
  ...
  function bindTrackRow(row, api, gpVideo, fullDuration, getWindowOffset, getDisplayDuration, ac) {
    ...
    const srcToOut = (s) => { const cs = _cl(); for (const c of cs) if (s >= c.sourceStart && s < c.sourceEnd) return c.outputStart + (s - c.sourceStart); if (cs.length && s <= cs[0].sourceStart) return 0; const l = cs[cs.length - 1]; return l ? l.outputStart + (l.sourceEnd - l.sourceStart) : s; };
    const outToSrc = (o) => { const cs = _cl(); for (const c of cs) { const oe = c.outputStart + (c.sourceEnd - c.sourceStart); if (o >= c.outputStart && o < oe) return c.sourceStart + (o - c.outputStart); } const l = cs[cs.length - 1]; return l ? l.sourceEnd : o; };
    ...
    const ro = new ResizeObserver(() => {
      ...
      bars.forEach(b => {
        ...
        if (c && masterDuration > 0) {
          ...
          b.width    = Math.max(2, Math.round(((c.sourceEnd - c.sourceStart) / masterDuration) * trackW));
