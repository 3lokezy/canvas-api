(function () {
  ...
  const _RENDER_SCRIPT_BUILD = 'pb-20260501h-backpressure';
  ...
  async function renderComposition(onProgress) {
    ...
    if (useMp4) {
      ...
      startMp4Output = async () => {
        ...
        output = new Output({
          // 'fragmented' writes moof+mdat fragments as they're encoded — no post-processing
          // rewrite needed at finalize time. This eliminates the finalization-time peak where
          // 'in-memory' mode needs to hold two copies of the full MP4 to reorder moov to front.
