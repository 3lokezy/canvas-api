(() => {
  ...
  function createEngine(gpVideo, cv, media) {
    ...
    const BG_BLUR_REF = 20 / 1080;   // render's blur radius as a fraction of output width
    ...
    function drawBgMirror() {
      ...
      if (!cw || !ch) return;
      const scale = Math.max(pw / cw, ph / ch) * 1.1;          // cover ×1.1, as render
      ...
      if (_CTX_FILTER_OK) {
        ...
        const bc = Math.max(bw / cw, bh / ch) * 1.1, bDW = cw * bc, bDH = ch * bc;
