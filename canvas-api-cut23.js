(function () {
  ...
  function buildPanelAPI(refs) {
    ...
    function _syncMusicToVideo() {
      ...
      if (drift > 0.2) {
        if (window.WC_MUSIC_DIAG) {
          console.log(`[music] seek ${_musicAudio.currentTime.toFixed(2)}→${targetTime.toFixed(2)} ` +
            `drift=${drift.toFixed(2)}  pos=${pos.toFixed(2)}  trackStart=${trackStart.toFixed(2)} offset=${_musicOffset.toFixed(2)}`);
        }
    ...
    // so drive music + watermark from here. On handback, let gpVideo's own state take over.
  ...
  function initCanvas(item, opts = {}) {
    ...
    if (splitHandle && gpHold && fcHold) {
      ...
      applySplit = pct => {
        ...
        // Stick the overlay placement bar to the split seam so it doesn't drift down
        // over the facecam (and the face). There are platform variants (kick/twitch +
        // a hidden one) all sharing the class — position every match so whichever is
        // visible rides the seam. % is relative to the full-frame .kick_twitch_overlay
        // → maps 1:1 to the frame (resize-safe). Bottom edge sits ON the seam (bar in
        // the gameplay half). Knob: translateY(-100%) = above seam, -50% = straddling,
        // 0 = top of the facecam.
        frame.querySelectorAll('.kt_overlay_placement_bar').forEach(_bar => {
          _bar.style.setProperty('top',       `${splitPct * 100}%`, 'important');
          _bar.style.setProperty('transform', 'translateY(-100%)',  'important');
        });
        applyGpZoom(gpZoom);
      };
