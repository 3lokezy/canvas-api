(function () {
  ...
  const _RENDER_SCRIPT_BUILD = 'pb-20260501c';
  ...
  async function ensureMp4Support() {
    ...
    if (_canMP4) {
      if (_useBufferedMp4Audio && typeof AudioEncoder === 'undefined') {
        console.log('Render: WebCodecs H.264 available; no AudioEncoder — video-only MP4 (Chrome iOS)');
      } else {
  ...
  async function renderComposition(onProgress) {
    ...
    // Chrome iOS has VideoEncoder but not AudioEncoder — we still use the Mediabunny path
    // for H.264 video; audio will be skipped (silent MP4) since AudioEncoder is unavailable.
    const useMp4 = webCodecsMp4Ok;
    console.log('Render: [3] encoding setup — useMp4:', useMp4, _isIOS ? '(iOS detected)' : '',
      useMp4 && typeof AudioEncoder === 'undefined' ? '— video-only (no AudioEncoder)' : '');
    let audioCtx = null;
