...
v.src = isLargeSource ? src : src + (src.includes('?') ? '&' : '?') + '_r=' + Date.now();
  ...
  async function renderComposition(onProgress) {
    ...
    const isInSource = effectiveStart > 0;
...
if (kickLogo && barEl) {
  ...
  if (bR) {
    ...
    const useMp4 = webCodecsMp4Ok && (typeof AudioEncoder !== 'undefined' || _isIOS || _isChromeIOS) && !isInSource;
    ...
    if (useMp4) {
      ...
      try {
        ...
        if (isInSource) {
          let _gpSrc = null;
          try { _gpSrc = audioCtx.createMediaElementSource(gpVideo); } catch (_) {}

          if (_gpSrc) {
            // Web Audio mixer — supports music mixing
          ...
          } else if (typeof gpVideo.captureStream === 'function') {
            // CORS blocks Web Audio on cross-origin VOD — direct captureStream (no music mixing)
            gpVideo.captureStream().getAudioTracks().forEach(t => stream.addTrack(t));
            console.warn('Render: using captureStream fallback — music mixing unavailable for cross-origin VOD');
          }
        } else {
    ...
    const vw = (isInSource ? gpVideo : rv).videoWidth;
    const vh = (isInSource ? gpVideo : rv).videoHeight;
      ...
      rv.addEventListener('seeked', async function onSeeked() {
        ...
        if (!useMp4) {
          ...
          if (frame._pendingMusicNode && audioCtx) {
            ...
            const trackEnd   = tracks.music?.end   ?? totalDuration;
        ...
        function drawCanvasFrame(t) {
          const _ds = isInSource ? gpVideo : rv;
          ctx.fillStyle = '#000';
