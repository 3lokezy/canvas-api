(function () {
  ...
  function bindTrackRow(row, api, gpVideo, fullDuration, getWindowOffset, getDisplayDuration, ac) {
    ...
    function getWindow() {
      ...
      return OUT_AXIS ? { start: srcToOut(raw.start), end: srcToOut(raw.end) } : raw;   // operate in output space
    }
    ...
    function applyWindow(start, end) {
      if (isMaster) {
        ...
        api.setTrack(name, { start: outToSrc(start), end: outToSrc(end) });   // output space → source storage
      } else {
