(function () {
  ...
  function buildPanelAPI(refs) {
    ...
    window.canvasAPI = {
      setMode(mode)            {
        ...
        if (id) {
          // Per-clip: break this clip away to its own mode; the sampler re-applies it (clipsChanged
          // forces a re-apply next frame). Other clips keep the base mode.
          const clips = (tracks.video.clips || []).map(c => c.id === id ? { ...c, override: { ...(c.override || {}), mode } } : c);
          this.setClips(clips);
        } else {
          applyModeFn(mode);   // base (global) mode → all inheriting clips follow
        }
      },
      ...
      getEditClip() {
        const id = document.querySelector('.track-active.is-selected[data-clip-id]')?.dataset.clipId;
        if (!id) return null;
        // The FIRST clip IS the parent / single source of truth: editing it updates the base
        // (all inheriting clips follow), so it never holds its own override. Clips 2+ break away.
        const first = (tracks.video.clips || []).slice().sort((a, b) => a.outputStart - b.outputStart)[0];
        return (first && first.id === id) ? null : id;
      },
  ...
  function initCanvas(item, opts = {}) {
      ...
      function _applyReframe(srcSec, ot, clip) {
        ...
        if (clip !== undefined) {
          ...
          if (cid !== _lastLayoutClipId) {
            ...
            const ovr = clip && clip.override;
            // MODE: switch only when the effective mode actually changes (reuses the tested inner
            // applyMode — class/facecam/split DOM swap WITHOUT touching the global modeStates).
            const effMode = (ovr && ovr.mode) || _baseMode;
            ...
            // LAYOUT: clip override → its layout; else if mode changed or leaving an override → base.
            const L = ovr && ovr.layout;
    ...
    let splitPct   = 0.5;
    let applySplit = null;
    let _baseMode  = 'is-full';   // the parent/base mode clips inherit (per-clip override.mode breaks away)
    ...
    function applyModeFn(mode) {
      _baseMode = mode;   // applyModeFn is the GLOBAL switcher → this is the base all clips inherit
