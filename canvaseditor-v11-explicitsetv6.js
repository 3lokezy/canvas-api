(function () {
  ...
  function initCanvas(item, opts = {}) {
    ...
    if (!interactive) {
        // Apply basic full mode so gameplay hold lays out correctly
        MODES.forEach(m => {
          frame.classList.remove(m);
          gpHold?.classList.remove(m);
          fcHold?.classList.remove(m);
  ...
  function reapplyCrop(item) {
    ...
    if (imageUrl) {
      ...
      if (!hasSaved) {
        if (window.canvasAPI) {
          ...
          // Passive mode — show image directly
          const imgHold = item.querySelector('.image_hold');
    ...
    if (cropValid) {
        ...
        if (item._applyMode) {
          ...
          // Passive mode — set classes directly
          MODES.forEach(m => { frame?.classList.remove(m); gpHold?.classList.remove(m); fcHold?.classList.remove(m); });
          const f = item.querySelector('.clip_canvas');
          const g = item.querySelector('.gameplay_hold');
          f?.classList.add('is-split');
        ...
        if (item._applyMode) {
          ...
          // Passive mode — ensure full layout
          const f = item.querySelector('.clip_canvas');
          const g = item.querySelector('.gameplay_hold');
          MODES.forEach(m => { f?.classList.remove(m); g?.classList.remove(m); fcHold?.classList.remove(m); });
          f?.classList.add('is-full');
          g?.classList.add('is-full');
          if (fcHold) fcHold.style.display = 'none';
        }
      }
  }
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  function bootCanvas() {
    // Interactive editor — single item
    ...
    // Passive clips — collection list, any number of items
    // initCanvas runs once, reapplyCrop runs every boot so Wized data is always fresh
