(function () {
  ...
  function buildPanelAPI(refs) {
    ...
    window.canvasAPI = {
      ...
      setMasterTrim(trimIn, trimOut) {
        ...
        // Clamp cuts to new effective bounds — remove any that now fall fully outside
      ...
      setTrack(name, { start, end } = {}) {
        ...
        // ── Cap music track to music file duration ──────────────────────────────
        if (name === 'music') {
  ...
  function initCanvas(item, opts = {}) {
    ...
    if (gpHold && gpVideo) {
      ...
      function createBgVideo() {
        ...
        if (typeof gpVideo.captureStream === 'function') {
          // Perfect sync — mirrors live decoded stream, zero independent network load
          bg.srcObject = gpVideo.captureStream();
          bg.play().catch(() => {});
        } else {
          // iOS fallback — separate src with manual sync
          bg.src     = gpVideo.src;
          bg.preload = 'auto';
    ...
    function _genClipId() { return `clip_${Date.now()}_${++_clipIdCounter}`; }
    ...
    gpVideo.addEventListener('timeupdate', () => {
      ...
      // NLE clip mode — canvas-clips.js owns enforcement when clips are active
